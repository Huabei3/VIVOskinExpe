...
lab_aft3=xyz2lab(XYZ_aft3,'d65_64');
...
%     plot(lab_CAT(i_para,2), lab_CAT(i_para,3),  'p', 'MarkerSize', 10, ...
...
%             atan2d(lab_CAT(i_para,3),lab_CAT(i_para,2)) , ...
...
% 
%     save_folder=fullfile("ellip_pic\ellipse_m_cat",Dtype);
...
%%
%draw L-C,a-b
output_folder=fullfile("ellip_pic\linear\m\CATed",Dtype);
...
for i_para=1:n_para
    ...
    scatter(L(i_para),C_all(i_para), 40, 'filled'); 
...
% saveas(gcf,fullfile(output_folder,'L_C.jpg'));
% 
% 
% %%
% %a-b
...
% for i_para=1:n_para
% 
%     scatter(lab_CAT(i_para,2),lab_CAT(i_para,3), 40, 'filled'); 
%     text( lab_CAT(i_para,2)+4,lab_CAT(i_para,3),strcat('{\it L*=}',num2str(L(i_para,1))), ...
...
% %绘制拟合直线
% axis equal;
% max_lim=max(max(lab_CAT(:,2)),max(lab_CAT(:,3)))+10;
% x = 0:0.1:max_lim;
% y= a_ab*x;
% plot(x,y,"Color","r");
% 
...
% ylabel('{\itb*}');
% title('{\ita*-b*}');
% saveas(gcf,fullfile(output_folder,'a_b.jpg'));
