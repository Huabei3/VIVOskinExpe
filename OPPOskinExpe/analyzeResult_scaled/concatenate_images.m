function concatenate_images(save_folder,n_col)
    % 读取输入目录中的所有JPG图片文件
    ...
    labels{i}=char(double('a') - 1 + 3);
    % labels{i}=image_files(round((i-1)/3)+1).name;
    % 设置文本的颜色和位置
    ...
    for i = 1:num_images
        ...
        % img_with_text = insertText(img, text_position_number,strcat("(",image_files(round((i-1)/3)+1).name,")"),  ...
        %     'FontSize', font_size_number, 'TextColor', text_color_number, 'BoxOpacity', 0);
