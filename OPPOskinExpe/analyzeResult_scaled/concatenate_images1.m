function concatenate_images1(save_folder,n_col)
    % 读取输入目录中的所有JPG图片文件
    image_files=dir(fullfile(save_folder,"a_b*.jpg"));
    image_files=[image_files;dir(fullfile(save_folder,"L_C*.jpg"))];
    image_files=[image_files;dir(fullfile(save_folder,"L_a*.jpg"))];
    image_files=[image_files;dir(fullfile(save_folder,"L_b*.jpg"))];
    ...
    labels=["a","b","c","d","e"];
    % 设置文本的颜色和位置
    text_color_number = [0, 0, 0]; % 红色 (R, G, B) - 用于图像编号
