...
for i_lastPart=1:length(lastParts)
    ...
    if lightness_type=="abs"
        source_folder=fullfile('AnalyseResults',lightness_type,Dtype,lastPart,'ellipPara_scaled');
    elseif lightness_type=="rela"
    ...
    %------保存参数------
    sori_labs_folder=fullfile('AnalyseResults',Dtype,'scene1','95',lightness_type);
    ...
    contour95ellip1(center,mu,lab_group,chi2_val, ...
        output_folder,lastPart,colors(i_lastPart,:),"scene1",labCh_ori); 
    if strcmp(lastPart,"indoorAdd")
        contour95ellip1(center_temp,mu,lab_group,chi2_val, ...
            output_folder,lastPart,colors(i_lastPart,:),"scene1",labCh_ori); 
    end
    labCh_center=center;
