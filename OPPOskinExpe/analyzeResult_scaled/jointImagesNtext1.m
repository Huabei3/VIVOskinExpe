...
%%
% 设置输入目录和输出目录
save_folder="D:\work\project_code_backup\OPPOskinExpe\analyzeResult_scaled\ExperimentResult\OPPOskin";
% save_folder="Z:\homes\Peggy\oppoSkinExperi\ExperimentResult\OPPOskin\1";
% save_folder="AlalyseResults\rpl_fm2\inLab\ellipsoid_CATed";
% save_folder="AlalyseResults\rpl_fm2\inLab\ellipsoid_sections\a_b";
% save_folder="D:\work\FirstYearMaster\oppoSkinExperi\analyzeResult\AlalyseResults\mNf\inLab\ellipsoid_sections\a_b";
...
% 读取第一张图片并获取其大小
first_image = imread(fullfile(input_folder, image_files(2).name));
%------获取dlab------
...
% text_color_number = [1, 1, 1].*255; % 橙色 (R, G, B) - 用于图像编号
model_name=["female1","female2","female3","female4","female5",...
...
for i = 1:num_images
    ...
    text_position_number = [cols - font_size_number*2.5, rows - font_size_number*1.5];
    text_position_labels = [cols - font_size_labels*15, rows - font_size_labels*1.5];

    % 在图片上插入编号
    % img_with_text=img;
    img_with_text = insertText(img, text_position_number, strcat("(",num2str(i),")"), ...
        'FontSize', font_size_number, 'TextColor', text_color_number, 'BoxOpacity', 0);
        % img_with_text = insertText(img, text_position_number, model_name(i), ...
        % 'FontSize', font_size_number, 'TextColor', text_color_number, 'BoxOpacity', 0);

    % 在图片上插入其他文本
    % img_with_text = insertText(img_with_text, text_position_labels, ...
    %     strcat(num2str(delta_lab(i,1)),',',num2str(delta_lab(i,2)),',',num2str(delta_lab(i,3))), ...
