function concatenate_figs_legend1(save_folder, figFiles, n_col, ...
    ...
    % 默认用 legendHeightNorm；设大一点（如0.15）可把所有子图往上推
    ...
    for i = 1:numFigs
        ...
        % 统一设置 xlabel/ylabel 字体大小（受 fontSizeScale 控制）
        if isfield(legend_labels,"label_fontSize")
            % 优先使用专门设置的 label_fontSize
        ...
        elseif isfield(legend_labels,"fontSizeScale")
            % 使用 fontSizeScale * targetFontSize
        ...
        if isfield(legend_labels,"if_label")&&legend_labels.if_label
            ...
            % 统一字体大小（受 fontSizeScale 控制）
            if isfield(legend_labels, "label_fontSize")
                letterFontSize = legend_labels.label_fontSize;
            elseif isfield(legend_labels, 'fontSizeScale')
                letterFontSize = legend_labels.fontSizeScale * targetFontSize;
            else
                letterFontSize = targetFontSize;
            end
            text(subAx, 0.95, 0.05, letter_label, 'Units', 'normalized', ...
                'FontSize', letterFontSize, 'FontWeight', 'bold', ...
...
function draw_legend_overlay(mainFig, legendPos, targetFontSize, legend_labels, label_Y)
    ...
    % 获取缩放后的图例字号（受 fontSizeScale 控制）
    if isfield(legend_labels, "label_fontSize")
        % 优先使用专门设置的 label_fontSize
        legendFontSize = legend_labels.label_fontSize;
    elseif isfield(legend_labels, 'fontSizeScale')
