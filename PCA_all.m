...
for i_obs = 1:length(obs_types)
    ...
    for i_nation = 1:length(nations)
        ...
        if ~isempty(data_matrix) && size(data_matrix, 2) == length(attributes)
            ...
            if size(valid_data, 1) > size(valid_data, 2) && size(valid_data, 2) > 1 % Need more observations than variables, and more than 1 variable
                ...
                saveas(gcf, fullfile(pca_img_folder, sprintf('PCA_%s_combined.jpg', nations(i_nation))));
                close(gcf);
            else
    ...
    %% Save PCA details for each nation to separate worksheets in a new Excel file
    % This part is modified to match your requested format
    pca_detailed_output_file = fullfile(pca_save_folder, strcat('combined_i_r_', obs_type, '_pca_detailed_output.xlsx'));
    ...
    for i_nation = 1:length(nations)
        if ~isempty(pca_results{i_nation})
            ...
            pcn_valid = pca_data.pcn_valid; % Valid attribute names
             
            % Initialize a cell array to build the sheet data column by column
            current_sheet_columns = {};
            
            % Create the "PC" row and "Explained Variance" row
            pc_numbers_row = arrayfun(@(x) sprintf('PC%d', x), 1:length(explained), 'UniformOutput', false);
            explained_str_row = arrayfun(@(x) sprintf(' %.2f%%', x), explained, 'UniformOutput', false);
            
            % Add an empty cell at the beginning of these rows for the "Attributes" header column later
            % pc_numbers_row = [{' '}, pc_numbers_row];
            % explained_str_row = [{' '}, explained_str_row];

            % Combine them to form the first two rows of the sheet
            header_rows = [pc_numbers_row; explained_str_row'];

            % Determine the maximum number of attributes to list (equal to number of valid attributes)
            max_attrs_per_pc = size(coeff, 1);
             
            % Prepare a cell array for attribute names (first column)
            
            % Initialize the main data part with the attribute names column
            sheet_data_main = {};
            
            % For each PC, sort attributes by absolute coefficient and add to sheet data
            for pc_idx = 1:size(coeff, 2)
                [sorted_abs_coeff, sort_idx] = sort(abs(coeff(:, pc_idx)), 'descend');
                 
                % Construct the column for this PC
                pc_column_data = cell(max_attrs_per_pc, 1);
                for attr_row_idx = 1:max_attrs_per_pc
                    pc_column_data{attr_row_idx} = sprintf('%s (%.4f)', pcn_valid{sort_idx(attr_row_idx)}, coeff(sort_idx(attr_row_idx), pc_idx));
                end
                
                % Append this PC's data column (including a blank for the "Attributes" header row)
                sheet_data_main = [sheet_data_main,  pc_column_data];
            end
            
            % Vertically concatenate header rows with the main data
            final_output_data = [header_rows; sheet_data_main];

            % Write to the Excel sheet
            sheet_name = nations_new(i_nation); % Use the new, descriptive nation name
            writecell(final_output_data, pca_detailed_output_file, 'Sheet', sheet_name);
        end
    end
end