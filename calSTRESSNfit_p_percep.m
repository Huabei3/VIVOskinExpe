...
for i_level=10:10:80
    ...
    scores=score_level';
    mean_scores = mean(scores, 2); % 16x1 的矩阵，每行代表一个被试的平均打分
    std_scores = std(scores, 0, 2); % 16x1 的矩阵，每行代表一个被试的标准差
    
    % 计算每个被试的 z-score
    z_scores = (scores - mean_scores) ./ std_scores; % 404x16 的矩阵，每行代表一个被试的 z-score
    
    % 计算所有被试的 z-score 平均值和标准差
    mean_z_scores = mean(z_scores, 1); % 16x1 的矩阵，每行代表一个被试的平均 z-score
    std_z_scores = std(z_scores, 0, 1); % 16x1 的矩阵，每行代表一个被试的标准差
    
    % 计算 Perceptibility 椭圆的参数
    perceptibility_center = mean_z_scores; % 椭圆中心
    perceptibility_cov = cov(z_scores); % 协方差矩阵
    perceptibility_inv_cov = inv(perceptibility_cov); % 协方差矩阵的逆矩阵
    
    % % 计算 Acceptability 椭圆的参数
    % acceptability_alpha = 2.5; % 拟合模型参数，根据实际数据进行调整
    
    % 使用 Perceptibility 椭圆模型进行拟合和绘制
    figure(i_level/10);
    hold on;
    axis equal;
    ellipse(perceptibility_center(1), perceptibility_center(2), ...
        sqrt(perceptibility_inv_cov(1,1)), sqrt(perceptibility_inv_cov(2,2)), ...
        atan2(perceptibility_inv_cov(2,1), perceptibility_inv_cov(1,2)), ...
        'EdgeColor', 'none', 'FaceColor', 'none');
    grid on;
    xlabel('a*');
    ylabel('b*');
    title(sprintf('Perceptibility Ellipse {\itL*}=%d',i_level));
    saveas(i_level/10,fullfile("ellip_pic\ellipse_percep", ...
        sprintf('Perceptibility Ellipse L*=%d',i_level)));
    % % 使用 Acceptability 椭圆模型进行拟合和绘制
    % figure;
    % hold on;
    % axis equal;
    % ellipse(acceptability_center(1), acceptability_center(2), sqrt(acceptability_inv_cov(1,1)), sqrt(acceptability_inv_cov(2,2)), atan2(acceptability_inv_cov(2,1), acceptability_inv_cov(1,2)), 'EdgeColor', 'none', 'FaceColor', 'none');
    % grid on;
    % xlabel('a*');
    % ylabel('b*');
    % title('Acceptability Ellipse');
...
% %拟合椭圆
% 
% for i_obs=9:length(dir_res)
%     par_ind=[];r_ind=[];
%     for i_level=30:10:80
%         load(strcat("level_data\labNscore_level_p",num2str(i_level),".mat"));
% 
...
% 
%     save(sprintf("ind_ellip\ind_ellip_para_%02d.mat",i_obs), ...
...
for i_obs=1:length(dir_res)
    ...
    par_ind_all{i_obs,1}=[par_ind;par];
    r_ind_all{i_obs,1}=[r_ind;r];
    load(strcat("ind_ellip\ind_ellip_para_",num2str(i_obs),".mat"),'par_ind','r_ind');
