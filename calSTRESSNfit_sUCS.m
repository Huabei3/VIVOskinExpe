...
Iab_1 = XYZ2sUCS(XYZ, XYZw_pre);
%分亮度保存
output_folder="level_data\sUCS";
if ~exist(output_folder,"dir")
    mkdir(output_folder);
end
for i_level=10:10:80
    lab_level=lab(lab(:,1)==i_level,:);
    Iab_level=Iab_1(lab(:,1)==i_level,:);
    score_level=score_all(lab(:,1)==i_level,:);
    I_mean=mean(Iab_level(:,1));
    save(fullfile(output_folder,strcat(sprintf("%02d",i_level),".mat")), ...
        'lab_level','score_level',"Iab_level","I_mean");
end
%%
%计算STRESS_inter
...
for i_level=10:10:80
    ...
    [par,r,y] = my_ellipsoidfit3(Iab_level,p);
    figure(i_level/10);
    plot_contour_with_scatter(par, Iab_level, p);
