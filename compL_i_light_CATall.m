...
for attribute = attributes
    ...
    if exist(source_file4, 'file') % 新增 source_file4 的加载
        ...
        if exist('par_all4', 'var')&&~isempty(par_all4)
            ...
            center_hml{1,1}=mean(centers_pre(1:7,:),1);%h
            center_hml{1,2}=mean(centers_pre(15:21,:),1);%m
            center_hml{1,3}=mean(centers_pre(8:15,:),1);%l
            ...
            for i_col=1:3
                for i_row=1:3
                    de_hml(i_col,i_row)=deltaE2000(center_hml{1,i_col},center_hml{1,i_row});
                end
            end
            for i_CT=1:7

                for i_hml=1:3
                    de_inter_hml(i_hml,i_CT)=deltaE2000(centers_hml{1,i_hml}(i_CT,:),center_hml{1,i_hml});
                end
            end
        end
        de_attrs{attribute,1}=de_hml;
        de_attrs{attribute,2}=de_inter_hml;
        de_attrs{attribute,3}=mean(mean(de_hml));
        de_attrs{attribute,4}=mean(mean(de_inter_hml));
        ...
        ylim([0, 20]);
            ...
            % 在图片右下角添加 de_attrs{attribute, 3} 和 de_attrs{attribute, 4} 的值
        text(18, 2, sprintf('dE between HML: %.2f\ndE between lights: %.2f', ...
        de_attrs{attribute, 3}, de_attrs{attribute, 4}), ...
