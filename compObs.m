...
for lastPart_idx = 1:length(lastParts)
    ...
    save_folder = fullfile("ellip_pic\ellipse", lastPart,Dtype,"compObs");
...
%% 循环处理每个 i_para（即每个 light）
    ...
    for attribute = attributes
        ...
        for i_para = 1:21
            ...
            % 定义路径
            source_file{1} = fullfile('AlalyseResults', Dtype,lastPart, attribute_serial, 'ellipPara', 'fitRes_level.mat');
            source_file{2} = fullfile('AlalyseResults', Dtype,lastPart, 'model_group', attribute_serial, 'ellipPara', 'fitRes_level.mat');
            source_file{3} = fullfile('AlalyseResults',Dtype, lastPart, 'model', attribute_serial, 'ellipPara', 'fitRes_level.mat');
            source_file{4} = fullfile('AlalyseResults', Dtype,lastPart, 'all', attribute_serial, 'ellipPara', 'fitRes_level.mat'); % 新增 source_file4
            for i_obs=1:4
                % 加载数据
                par_all_used = [];
                ...
                if ~isempty(par_all_used)
                    ...
                    contour(data2, data3, y, [0.5, 1], 'Linewidth', 1, 'Color', colors(i_obs, :), ...
                         'LineStyle', '-');
                    scatter(par(4), par(5), 30, 'v', 'filled', 'MarkerFaceColor', colors(i_obs, :));
                end
            end
            % dE_intra_obs=
            par_cell{lastPart_idx,attribute,i_para}=par;
            % 绘制 PMCC 点
            ...
            exportgraphics(gcf, fullfile(save_folder, attribute_serial, strcat(picname_group(i_para),  '.jpg')), 'Resolution', 300);
            close(gcf);
        end
