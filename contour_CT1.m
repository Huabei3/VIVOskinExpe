...
for i_nation=1:length(nations)
        ...
        for i_attr = 1:1
            ...
            logic_idx = (all_ethnicities == string(nation)) & ...
                    (all_attributes == attribute)&(all_observer_types == "non_model");
                
            filtered_rows1 = VIVO_table(logic_idx, :);
            all_CCTs=vertcat(filtered_rows1.CCT{:});
            lim_CCT=[min(all_CCTs),max(all_CCTs)];

            for i_scene=1:length(picnames_groups)

                all_scenes = string(filtered_rows1.scene);
                logic_idx = (endsWith(all_scenes,picnames_groups(i_scene)));                    
                filtered_rows = filtered_rows1(logic_idx, :);
                scene_CCTs=vertcat(filtered_rows.CCT{:});
                mean_CCT=mean(scene_CCTs);
                ...
                if size(filtered_rows,1)>0
                    ...
                    lab_cens_scene=vertcat(filtered_rows.lab_cen_scaled{:});
                    lab_mean_scene(i_scene,:)=mean(lab_cens_scene,1);
        ...
        save_file=fullfile(save_folder,strcat(nation_serial,"_contour_CT.mat"));
        save(save_file,"parNr","lab_group","p_group","par_ave","lab_mean_scene","filtered_rows");
end


%% --- 独立循环：计算所有颜色 (Colors Calculation) ---
% 在处理数据之前，先根据全局 CCT 范围定好颜色
colors_nations = cell(length(nations), 1);
cmap_resolution = 256;
current_cmap = flipud(turbo(cmap_resolution));
% all_CCTs = vertcat(VIVO_table.CCT{:});
% lim_CCT = [min(all_CCTs), max(all_CCTs)];
lim_CCT=[2500,8500];
for i_nation = 1:length(nations)
    nation = nations(i_nation);
    % 筛选该 Nation 的所有数据以获取 CCT 极值
    logic_idx_cct = (string(VIVO_table.model_ethnicity) == nation) & ...
                    (string(VIVO_table.attribute) == attributes(1)) & ...
                    (string(VIVO_table.observer_type) == "non_model");
    tmp_table = VIVO_table(logic_idx_cct, :);
    
    if ~isempty(tmp_table)
        % 计算该 Nation 下每个场景的颜色
        current_colors = zeros(length(picnames_groups), 3);
        for i_scene = 1:length(picnames_groups)
            scene_rows = tmp_table(endsWith(string(tmp_table.scene), picnames_groups(i_scene)), :);
            if ~isempty(scene_rows)
                mean_CCT = mean(vertcat(scene_rows.CCT{:}));
                % 归一化插值
                norm_val = (mean_CCT - lim_CCT(1)) / (lim_CCT(2) - lim_CCT(1));
                norm_val = max(0, min(1, norm_val));
                current_colors(i_scene, :) = interp1(linspace(0, 1, cmap_resolution), current_cmap, norm_val);
            else
                current_colors(i_scene, :) = [0.5 0.5 0.5]; % 缺省灰色
            end
        end
        colors_nations{i_nation} = current_colors;
    end
end
% 3. 配置全局 Colorbar (类似你提供的代码逻辑)
...
clim([lim_CCT(1), lim_CCT(2)]); 

% 优化坐标轴显示
set(gca, 'XTick', 1:length(nations), 'XTickLabel', nations, ...
         'YTick', 1:length(picnames_groups), 'YTickLabel', picnames_groups, ...
         'TickLabelInterpreter', 'none', 'YDir', 'reverse');
title('Scene Color Mapping by CCT');
grid on;
%%
...
%     if i_nation~=4
        ...
        nation_limits(i_nation).lim_max_x = 28;
        nation_limits(i_nation).lim_min_y = 0;
        nation_limits(i_nation).lim_max_y = 28;
%     else
%         nation_limits(i_nation).lim_min_x = 0;
    ...
    for i_attr = 1:1
        ...
        for i_scene=1:length(picnames_groups)
            ...
            if ~isempty(a)&&sum(a(2:3))~=0
                ...
                % 绘制等高线
                color=colors_nations{i_nation}(i_scene,:);
...
%%
s.labels_row1 = {"in-lab","real scene","PMCC"};
s.colors_row1=[0 0 0;0 0 0; 1 0 1];
s.plot_style_row1={'o','^','s'};
s.labels_row2 = {};
s.markers_row2 = {};
s.markers_colors = [];
