...
for i_nation=1:length(nations)
        ...
        save_folder = fullfile('AnalyseResults_p', Dtype, ...
            scale_type ,"contour_attr",obs_type_used);
        ...
        for i_attr = 1:length(attributes)
            ...
            if size(filtered_rows,1)>0
                ...
                par_ave(i_attr,:)=mean(par_group,1,'omitnan');
        ...
        save_file=fullfile(save_folder,strcat(nation_serial,"_contour_attr.mat"));
...
for i_nation = 1:length(nations)
    ...
    % 遍历每个属性绘制椭圆
    for i_attr = 1:size(data_nation.parNr,1)
        attribute = attributes(i_attr);
        if strcmp(par_type,"merge_fit")
            a = data_nation.parNr(i_attr, 1:end-1);
        elseif strcmp(par_type,"ave")
            a = data_nation.par_ave(i_attr, :);
        end
