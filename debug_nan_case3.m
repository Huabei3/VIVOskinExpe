...
if sum(valid_tr_mask) == 0
    ...
    for i = 1:size(lab_train_g, 1)
        fprintf('row %d: lab_nan=%s, par_nan=%s\n', i, ...
            mat2str(any(isnan(lab_train_g(i,:)))), ...
            mat2str(any(isnan(par_train_g(i,:)))));
    end
    
    % 检查原始 fitRes.mat 文件是否存在数据
    fprintf('\n检查源文件是否存在:\n');
    for ti = 1:length(train_indices)
        subject_idx = curr_nation_indices(train_indices(ti));
        lastPart = lastParts{subject_idx};
        src_file = fullfile('AnalyseResults_p', 'efit_p', 'unscaled', ...
        ...
        if exist(src_file, 'file')
            ...
            % 检查目标索引的行
            for it = 1:length(indices_target)
                idx = indices_target(it);
                if idx <= size(pd.par_all, 1)
                    fprintf('    target %d (pcn idx %d): %s\n', it, idx, mat2str(pd.par_all(idx,:)));
                else
                    fprintf('    target %d (pcn idx %d): out of bounds\n', it, idx);
                end
            end
        else
            fprintf('  文件不存在\n');
    ...
    for ti = 1:length(train_indices)
        ...
        lastPart = lastParts{subject_idx};
        avg_file = fullfile("aveSkin", lastPart, "autoNhand_scaleoverLUT.mat");
        fprintf('average 文件 %s: %s\n', lastPart, avg_file);
        if exist(avg_file, 'file')
            avg_d = load(avg_file);
            fprintf('  average_lab_all size: %s\n', mat2str(size(avg_d.average_lab_all)));
            for it = 1:length(indices_target)
                idx = indices_target(it);
                if idx <= size(avg_d.average_lab_all, 1)
                    fprintf('    target %d (pcn idx %d): %s\n', it, idx, mat2str(avg_d.average_lab_all(idx,1:3)));
                else
                    fprintf('    target %d (pcn idx %d): out of bounds\n', it, idx);
                end
            end
