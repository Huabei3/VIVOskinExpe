...
save_folder = fullfile("ellip_pic\ellipse",Dtype, "combined", "single");
...
[lim_min, lim_max] = calculate_global_limits(lastParts, attributes, attribute_names);
        ...
        for lastPart_idx = 1:length(lastParts)
            ...
            if ~isempty(par_all1)
                ...
                contour(data2, data3, y, [0.5, 1], 'Linewidth', 1, 'Color', colors(lastPart_idx, :), ...
                    'DisplayName', strcat(lastPart, ' - source1'), 'LineStyle', line_styles{1});
                scatter(par(4), par(5), 30, 'filled', 'MarkerFaceColor', colors(lastPart_idx, :));
            end
            ...
            if ~isempty(par_all2)
                ...
                y = (1 ./ (1 + par(6) * exp(sqrt(par(1) * (data2 - par(4)).^2 + par(2) * (data3 - par(5)).^2 + ...
                    ...
                    par(2) * (data3 - par(5)).^2 + par(3) * (data2 - par(4)) .* (data3 - par(5))) >= 0);
                contour(data2, data3, y, [0.5, 1], 'Linewidth', 1, 'Color', colors(lastPart_idx, :), ...
                    'DisplayName', strcat(lastPart, ' - source2'), 'LineStyle', line_styles{2});
                scatter(par(4), par(5), 30, '^', 'filled', 'MarkerFaceColor', colors(lastPart_idx, :));
            end
            ...
            if ~isempty(par_all3)
                ...
                if all(par([1:3, 6]) == 0)
                    ...
                    scatter(par(4), par(5), 30, 'd', 'filled', 'MarkerFaceColor', colors(lastPart_idx, :));
                else
                    % 否则，使用 ':' 线型绘制等高线图
                    check_data2 = par(4) + (-30:0.2:30);
                    check_data3 = par(5) + (-30:0.2:30);
                    ...
                    y = (1 ./ (1 + par(6) * exp(sqrt(par(1) * (data2 - par(4)).^2 + par(2) * (data3 - par(5)).^2 + ...
                        ...
                        par(2) * (data3 - par(5)).^2 + par(3) * (data2 - par(4)) .* (data3 - par(5))) >= 0);
                    contour(data2, data3, y, [0.5, 1], 'Linewidth', 1, 'Color', colors(lastPart_idx, :), ...
                        'DisplayName', strcat(lastPart, ' - source3'), 'LineStyle', line_styles{3});
                    scatter(par(4), par(5), 30, 'd', 'filled', 'MarkerFaceColor', colors(lastPart_idx, :));
                end
            end
            ...
            if ~isempty(par_all4)
                ...
                y = (1 ./ (1 + par(6) * exp(sqrt(par(1) * (data2 - par(4)).^2 + par(2) * (data3 - par(5)).^2 + ...
                    ...
                    par(2) * (data3 - par(5)).^2 + par(3) * (data2 - par(4)) .* (data3 - par(5))) >= 0);
                contour(data2, data3, y, [0.5, 1], 'Linewidth', 1, 'Color', colors(lastPart_idx, :), ...
                    'DisplayName', strcat(lastPart, ' - source4'), 'LineStyle', line_styles{4});
