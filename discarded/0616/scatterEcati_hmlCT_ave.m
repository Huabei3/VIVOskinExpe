...
picname_group=["h3k","h4k","h5k","hd65","h6k","h7k","h8k",...
...
for i_obstype=1:length(obs_types)
    save_folder = fullfile("ellip_pic\ellipse1",  Dtype, ...
        "scatterCen","hml_CT","average",obs_types(i_obstype));
    % 创建保存文件夹
    ...
    for lastPart_idx = 1:length(lastParts)
        ...
        for attribute = attributes
            ...
            source_file_used = fullfile('AnalyseResults1', Dtype, lastPart, ...
                obs_types(i_obstype), attribute_serial, 'ellipPara', 'fitRes.mat'); 
    ...
    %a-b
    lim_min_x=min(min(min(par_all4(:,4,:,:))))-1;
    lim_max_x=max(max(max(par_all4(:,4,:,:))))+1;
    lim_min_y=min(min(min(par_all4(:,5,:,:))))-1;
    lim_max_y=max(max(max(par_all4(:,5,:,:))))+1;
    ...
    for attribute = attributes
        ...
        for i_level = 1:3
            ...
            if exist('par_all_used', 'var')&&~isempty(par_all_used)
                ...
                text(par(:,4),par(:,5),  picname_group(idx_ranges(i_level,:)), ...
                    'Color',colors(i_level, :),'FontSize', 8, 'VerticalAlignment', 'middle');
                % scatter(par(:,4),par(:,5),  30, 'o', 'filled', ...
                %     'MarkerFaceColor', colors(i_level, :));
            end
        end
                    % 绘制 PMCC 点
        plot(labC_PMCCpre(5, 2), labC_PMCCpre(5, 3), 's', 'MarkerSize', 10, ...
            'MarkerFaceColor', 'm', 'MarkerEdgeColor', 'm');
        text(labC_PMCCpre(5, 2), labC_PMCCpre(5, 3),'h-PMCC', ...
            'FontSize', 8, 'VerticalAlignment', 'middle');

        plot(labC_PMCCpre(12, 2), labC_PMCCpre(12, 3), 's', 'MarkerSize', 10, ...
            'MarkerFaceColor', 'm', 'MarkerEdgeColor', 'm');
        text(labC_PMCCpre(12, 2), labC_PMCCpre(12, 3),'l-PMCC', ...
            'FontSize', 8, 'VerticalAlignment', 'middle');

        plot(labC_PMCCpre(19, 2), labC_PMCCpre(19, 3), 's', 'MarkerSize', 10, ...
            'MarkerFaceColor', 'm', 'MarkerEdgeColor', 'm');
        text(labC_PMCCpre(19, 2), labC_PMCCpre(19, 3),'m-PMCC', ...
            'FontSize', 8, 'VerticalAlignment', 'middle');
...
% 定义图例的 7 个颜色块和对应的名字
legend_names = ["h", "m", "l"];
legend_colors = [[1,0,0];[0,0,1];[0,1,0];]; % 使用 hsv 色图生成 7 种颜色
