...
Dtype='summer';
save_folder = fullfile("ellip_pic\ellipse",  Dtype,"scatterCen","hml","average");
% 创建保存文件夹
...
% save("D:\work\VIVOskinExpe\renderCode\aveSkinByHand2\average_mods\i\average_allMod_2.mat","average");
%a-b
lim_min_x=min(min(min(par_all4([7,14,21],4,:,:))))-1;
lim_max_x=max(max(max(par_all4([7,14,21],4,:,:))))+1;
lim_min_y=min(min(min(par_all4([7,14,21],5,:,:))))-1;
lim_max_y=max(max(max(par_all4([7,14,21],5,:,:))))+1;
...
% lim_min_x=min(min(min(C([7,14,21],1,:,:))))-1;
% lim_max_x=max(max(max(C([7,14,21],1,:,:))))+1;
% lim_min_y=min(min(min(average([7,14,21], 1))))-1;
% lim_max_y=max(max(max(average([7,14,21], 1))))+1;
% % for attribute = [10]
% for attribute = attributes
%     fprintf('Processing attribute: %d\n', attribute);
%     par_all_used=par_all4(:,:,1,attribute);
