    ...
    for i_CT = 1:7
        ...
        for i_nation_sheet = 5:5 % Modified to process only i_nation = 5
            if ~isempty(pca_results{i_nation_sheet})
                ...
                % Save coeff matrix to a separate Excel file, and add max loading attributes
                if ~isempty(coeff)
                    ...
                    % Combine header, data, and the new row
                    coeff_output_data = [coeff_table_headers; coeff_table_data; max_pcn_row];
