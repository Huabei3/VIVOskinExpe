...
%% 保存
output_folder=fullfile("ellip_pic_p_free", Dtype,CT_type);
...
for i_obs = 1:length(obs_types)
    ...
    for idx_attribute = 1:length(attributes) % 使用新的索引来遍历 attributes 数组
        ...
        for i_nation = 1:length(nations)
            ...
            % --- 调用建模函数并保存结果 ---
            % C_L 建模
            pre_stored=fullfile("AnalyseResults_p_free",Dtype, ...
                "unscaled\model_fullpara\i\non_model", ...
                strcat(attribute_serial,"_all_curve_params.mat"));
            r_C_L=pre_stored.r_CL_all(i_nation);
            RMSE_C_L=pre_stored.rmse_CL_all(i_nation);
            a_C_L=pre_stored.a_CL_all(i_nation, :);

            r_long_axis=pre_stored.r_long_axis_all(i_nation);
            RMSE_long_axis=pre_stored.rmse_long_axis_all(i_nation);
            a_long_axis=pre_stored.a_long_axis_all(i_nation, :);

            r_short_axis=pre_stored.r_short_axis_all(i_nation);
            RMSE_axis=pre_stored.rmse_short_axis_all(i_nation);
            a_short_axis=pre_stored.a_short_axis_all(i_nation, :);
            % 色调角建模
            r_hue_angle=pre_stored.r_hue_angle_all(i_nation);
            RMSE_angle=pre_storedrmse_hue_angle_all(i_nation);
            a_hue_angle=pre_stored.a_hue_angle_all(i_nation, :);
            % 椭圆倾角 theta 建模
            r_theta=pre_stored.r_theta_all(i_nation);
            RMSE_theta=pre_stored.rmse_theta_all(i_nation);
            a_theta=pre_stored.a_theta_all(i_nation,:); 
            
            r_alpha=pre_stored.r_alpha_all(i_nation);
            RMSE_alpha=pre_stored.rmse_alpha_all(i_nation);
            a_alpha=pre_stored.a_alpha_all(i_nation, :);
        ...
        % 保存拟合参数
        output_folder_params = fullfile('AnalyseResults_p_free', Dtype,scale_type_origin,"model_fullpara",CT_type, iOr, obs_type);
