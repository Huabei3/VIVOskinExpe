...
for i_model=1:length(lastParts)
    models{i_model}=strrep(lastParts{i_model},iOr,"");
end
if iOr =='i'
...
scenes=["h3k","h4k","h5k","h6k","hd65","h7k","h8k",...
            "m3k","m4k","m5k","m6k","md65","m7k","m8k",...
             "l3k","l4k","l5k","l6k","ld65","l7k","l8k",...
...
for i_nation=1:length(nations)
        ...
        for i_attr = 1:1
            ...
            for i_scene=1:length(scenes)
                ...
                logic_idx =  endsWith(lower(all_scenes),scene);
        ...
        save(save_file,"parNr","lab_group","p_group","filtered_rows");
end
...
for i_nation = 1:length(nations)
    if i_nation~=4
        ...
        nation_limits(i_nation).lim_max_x = 30;
        nation_limits(i_nation).lim_min_y = 0;
        nation_limits(i_nation).lim_max_y = 30;
    else
        nation_limits(i_nation).lim_min_x = 0;
        nation_limits(i_nation).lim_max_x = 16;
        nation_limits(i_nation).lim_min_y = 0;
        nation_limits(i_nation).lim_max_y = 16;
    end
end
...
for i_nation = 1:length(nations)
    ...
    % 遍历每个属性绘制椭圆
    for i_scene = 1:size(data_nation.parNr,1)
        scene = scenes(i_scene);
