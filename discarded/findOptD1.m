    ...
    for lastPart_idx = 1:length(lastParts)
        ...
        for attribute = [1]
            ...
            for i_para=1:length(picname_group)
                labNgroup_file=fullfile("AnalyseResults1",Dtype,lastPart, ...
...
% % D_CCT_del([6,13,20],:)=[];
% figure();hold on;
% for i_para=1:length(D_CCT_del)
%     i_shape=floor((i_para-1)/6)+1;
%     color_idx=mod(i_para,6)+1;
%     plot(D_CCT_del(i_para,2),D_CCT_del(i_para,1),shapes{i_shape}, ...
%         'MarkerFaceColor', colors(color_idx, :), ...
%             'MarkerEdgeColor', colors(color_idx, :));
% end
%%
...
x_fit=1116./x_fit;
y_fit=D_CCT_del(:,1);
[fitresult, gof] = createFit_poly(x_fit, y_fit);

a=[4.006e-12,-5.797e-08,0.000197, 0.7375];%灰卡4CCTest色温拟合
y_pre = a(1)*x_fit.^3 + a(2)*x_fit.^2 + a(3)*x_fit + a(4);
figure();hold on;
...
for i_row=1:length(x)
    D_poly3(i_row,:) = a(1)*x(i_row,:).^3 + a(2)*x(i_row,:).^2 + a(3)*x(i_row,:) + a(4);
...
XYZ_bf=lab2xyz2(lab_fit,'d65_64');
for i_para=1:size(lab_fit,1)
    D_pre(i_para,1)= calculateD(CT(i_para,1), 0, "poly3",E);
    XYZt = CAT16_D(XYZ_bf(i_para,:), ...
