        ...
        for attribute = attributes
            ...
            for lastPartIdx = 1:length(lastParts)
                ...
                % for i_para=target_indices{i_indice}                     
                    ...
                    h1=figure(i_indice*3+1);hold on;
                    if iOr=='i'
                    ...
                    h2=figure(i_indice*3+2);hold on;
                    if iOr=='i'
                ...
                % Remove colorbars as colors are now discrete based on lastPart
                figure(i_indice*3+1);
            ...
            if iOr=='i'
                output_folder = fullfile(save_folder,"color_chart1",iOr,obs_type,"a_b",hmls(i_indice));
            else
                output_folder = fullfile(save_folder,"color_chart1",iOr,obs_type,"a_b");
            end
            if ~exist(output_folder, "dir")
                mkdir(output_folder);
            end
            ...
            close(h1)
            if attribute==10
                concatenate_images_chart(output_folder,4);
            end
            
            h2=figure(i_indice*3+2);
            grid on;
            axis equal;
            if iOr=="i"
                max_lim=80;
            ...
            if iOr=='i'
                output_folder = fullfile(save_folder,"color_chart1",iOr,obs_type,"L_C",hmls(i_indice));
            else
                output_folder = fullfile(save_folder,"color_chart1",iOr,obs_type,"L_C");
            end
            if ~exist(output_folder, "dir")
                mkdir(output_folder);
            end
