function [lab_group_sing,MSV_group,picname_check]=process_data_ind(dir_res, lastPart,Dtype,data_type)
    % 处理数据并保存
    ...
    for i_group = 1:33:length(score_all)
        ...
        idx=floor((i_group - 1) / 33) + 1;
        picname_check{idx, 1} = picname_lab{i_group}(1:end - 3);
        lab_group = [];
        ...
        MSV_group{idx,1} = MSV_scaled(i_group:i_group + 32, :);
        ...
        for i_points=1:size(lab_group,1)
            ...
            if ~strcmp(Dtype,'noCAT')
                ...
                lab_group_CATed(i_points, :) = xyz2lab(XYZ_aft(i_points, :), 'd65_64');
            end

        end

        lab_group_sing{idx,1}=lab_group_CATed;
