...
save_folder = fullfile("ellip_pic\ellipse",  Dtype, ...
    "scatterCen","CT",obs_types(i_obstype),lastPart);
...
for i_obstype=1:4
    ...
    for attribute = attributes
        ...
        source_file{4} = fullfile('AnalyseResults',  Dtype,lastPart, 'all', attribute_serial, 'ellipPara', 'fitRes_level.mat'); 
        
        % 加载数据
    
        if exist(source_file{i_obstype}, 'file') % 新增 source_file4 的加载
            load(source_file{i_obstype});
            par_all4 = par_all;
        else
            par_all4=[];
        end
            ...
            xlabel('{\ita*}');
            ylabel('{\itb*}');
            title(strcat(obs_types_new(i_obstype),attribute_names(attribute)));
