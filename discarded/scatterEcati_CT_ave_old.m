...
Dtype='summer';
for i_obstype=1:4
    save_folder = fullfile("ellip_pic\ellipse", ...
        Dtype,"scatterCen","CT","average",obs_types(i_obstype));
    % 创建保存文件夹
    ...
    for attribute = attributes
            ...
            % for i_para = 1:length(par_all4)
                ...
                if exist('par_all4', 'var')&&~isempty(par_all4)
                    ...
                    scatter(par(4), par(5), 30, 'o', 'filled', 'MarkerFaceColor', colors(i_para, :));
                end         
            end
                        % 绘制 PMCC 点
            plot(mean(labC_PMCCpre(1:7, 2)), mean(labC_PMCCpre(1:7, 3)), 's', 'MarkerSize', 10, ...
                'MarkerFaceColor', 'm', 'MarkerEdgeColor', 'm');
            text(mean(labC_PMCCpre(1:7, 2)), mean(labC_PMCCpre(1:7, 3)),'PMCC', ...
    ...
    lim_min_x=min(min(min(C(1:7,1,:,:))))-1;
    lim_max_x=max(max(max(C(1:7,1,:,:))))+1;
    lim_min_y=min(min(min(average(1:7, 1))))-1;
    lim_max_y=max(max(max(average(1:7, 1))))+1;
    for attribute = attributes
            ...
            % for i_para = 1:length(par_all4)
                ...
                % 绘制 source_file_used 的 contour 或散点图 (新增部分)
                text(C(i_para,1,1,attribute),average(i_para, 1),  picname_group(i_para), 'FontSize', 8, 'VerticalAlignment', 'middle');
                scatter(C(i_para,1,1,attribute),average(i_para, 1),  30, 'o', 'filled', 'MarkerFaceColor', colors(i_para, :));
          
            end
                        % 绘制 PMCC 点C
            plot(mean(labC_PMCCpre(1:7, 4)), mean(labC_PMCCpre(1:7, 1)), 's', 'MarkerSize', 10, ...
                'MarkerFaceColor', 'm', 'MarkerEdgeColor', 'm');
            text(mean(labC_PMCCpre(1:7, 4)), mean(labC_PMCCpre(1:7, 1)),'PMCC', ...
                'FontSize', 8, 'VerticalAlignment', 'middle');
            % 添加图例、标签和标题
            xlabel('{\itC*}');
            ylabel('{\itL*}');
            title(strcat(obs_types_new(i_obstype),attribute_names(attribute),'L-C'));
            ...
            exportgraphics(gcf, fullfile(save_folder, 'L_C',strcat(attribute_serial,  'L-C.jpg')), 'Resolution', 300);
            close(gcf);
    end
