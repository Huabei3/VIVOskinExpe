...
average{1}=nanmean(average_inds,3);
par_all4{2}=nanmean(par_inds,3);

lim_min_x=min(min(min(par_all4(7,4,:,:))))-1;
lim_max_x=max(max(max(par_all4(7,4,:,:))))+1;
lim_min_y=min(min(min(par_all4(7,5,:,:))))-1;
lim_max_y=max(max(max(par_all4(7,5,:,:))))+1;
figure;
hold on;
set(gcf, 'Color', 'white');
for attribute = attributes
    ...
    attribute_serial = strcat(sprintf("%02d", attribute), attribute_names(attribute));
        ...
        % for i_para = 1:21
            ...
            if average(i_para, 1) <= 60
                C_pre(i_para, 1) = 6.7421 * log(average(i_para, 1)) - 9.9816; % 亮度实验
            else
...
exportgraphics(gcf, fullfile(save_folder, strcat( 'scatterEcat_attr_a_b.jpg')), 'Resolution', 300);
close(gcf);


%L-a
C=sqrt(par_all4(:,4,:,:).^2+par_all4(:,5,:,:).^2);
lim_min_x=min(min(min(C(7,1,:,:))))-1;
lim_max_x=max(max(max(C(7,1,:,:))))+1;
lim_min_y=min(average(7,1))-1;
lim_max_y=max(average(:,1))+1;
figure;
hold on;
set(gcf, 'Color', 'white');
for attribute = attributes
    ...
    attribute_serial = strcat(sprintf("%02d", attribute), attribute_names(attribute));
        ...
        for i_para = [7]
            ...
            if exist('par_all4', 'var')&&~isempty(par_all4)
                par = par_all4(i_para, :,attribute);                
                scatter(C(i_para,1,1,attribute), average(i_para,1), 30, 'o','filled', ...
                    'MarkerFaceColor', colors(attribute, :));
                attribute_char=char(attribute_serial);
                text(C(i_para,1,1,attribute), average(i_para,1), ...
                    attribute_char(1:2), 'FontSize', 7, ...
                    'VerticalAlignment', 'middle');
            end
...
exportgraphics(gcf, fullfile(save_folder, strcat( 'scatterEcat_attr_L_a.jpg')), 'Resolution', 300);
close(gcf);
%% 绘制图例图
figure;
hold on;
