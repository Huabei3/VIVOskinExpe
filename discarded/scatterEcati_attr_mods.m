...
for i_obstype=1:4
    ...
    for lastPart_idx = 1:length(lastParts)
        lim_min_x=min(lim_min_x,min(min(min(par_all4{lastPart_idx}(7,4,:,:)))))-1;
        lim_max_x=max(lim_max_x,max(max(max(par_all4{lastPart_idx}(7,4,:,:)))))+1;
        lim_min_y=min(lim_min_y,min(min(min(par_all4{lastPart_idx}(7,5,:,:)))))-1;
        lim_max_y=max(lim_max_y,max(max(max(par_all4{lastPart_idx}(7,5,:,:)))))+1;
    end
    for lastPart_idx = 1:length(lastParts)
        lastPart = lastParts{lastPart_idx};
        figure;
