    ...
    for i_nation = 1:length(nations)
        ...
        for i_subject = 1:n_subjects
            ...
            for i_attr = 1:length(attributes)
                ...
                source_file = fullfile('AnalyseResults_p_free', Dtype, ...
...
%% 保存加载的数据
output_folder_base = fullfile("ellip_pic_p_free", Dtype);
...
for i_obs = 1:length(obs_types_plotting)
    ...
    for attribute = attributes
        ...
        for i_nation = 1:length(nations)
            ...
            if iOr == 'i' && ~isempty(r_data_loaded) && ~isempty(afinal_i) % 确保i数据拟合成功
                ...
                %加载r的para
                file_C_Lpara_r=fullfile("ellip_pic_p_free\efit_p_free\C_L_iNr\r",obs_type, ...
                ...
                if ~isempty(lab_g_valid_r)
                    ...
                    if length(L_valid_r) > 1 % 至少需要2个点来计算相关性
                        ...
                        scatter(C_valid_r, L_valid_r, 8, colors(i_nation, :), scatter_style_r, 'filled', 'MarkerFaceAlpha', 0.6); 
                    end
                end
            end
        ...
        xlim([0, 35]);
        % 保存图片
        output_folder_curves = fullfile(output_folder_base, 'C_L_iNr', obs_type,iOr, 'curve_fit');
        if ~exist(output_folder_curves, 'dir')
            mkdir(output_folder_curves);
        end
        exportgraphics(h, fullfile(output_folder_curves, strcat(attribute_serial, '.jpg')), 'Resolution', 300);
        ...
        close(h); % 关闭当前图窗，避免内存占用过多
    end % end for attribute
    concatenate_images1(output_folder_curves, 5);
    % 导出当前i_obs的所有指标到XLSX文件（新增rho和promi工作表）
    r_excel_output_folder = fullfile("AnalyseResults_p_free",Dtype,scale_type_origin,"sum_list", ...
        'C_L', 'check_iNr');
    ...
    fprintf('已将 %s 的Pearson相关、RMSE、Spearman相关（rho）、显著性（promi）写入 Excel 文件。\n', obs_type);
end % end for i_obs