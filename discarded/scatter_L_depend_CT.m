...
for i_CT=1:7
    CT_7(i_CT,1)=mean(CCT_combi([i_CT,i_CT+7,i_CT+14],:));
end
...
for i_obs = 1:length(obs_types)
    ...
    for idx_attribute = 1:length(attributes) % 使用新的索引来遍历 attributes 数组
        ...
        for i_nation = 1:length(nations)
            ...
            % C_L 建模
                    % 保存图片
            output_folder = fullfile('ellip_pic_p_free', Dtype, "fullpara",CT_type,'C_L', iOr);
            if ~exist(output_folder, 'dir')
                mkdir(output_folder);
            end
            ...
            % 长轴建模
            output_folder = fullfile('ellip_pic_p_free', Dtype, "fullpara",CT_type,'long_axis', iOr);
            if ~exist(output_folder, 'dir')
                mkdir(output_folder);
            end
            ...
            % 短轴建模
            output_folder = fullfile('ellip_pic_p_free', Dtype, "fullpara",CT_type,'short_axis', iOr);
            if ~exist(output_folder, 'dir')
                mkdir(output_folder);
            end
            ...
            % 色调角建模
            output_folder = fullfile('ellip_pic_p_free', Dtype, "fullpara",CT_type,'hue_angle', iOr);
            if ~exist(output_folder, 'dir')
                mkdir(output_folder);
            end
            ...
            % 椭圆倾角 theta 建模
            output_folder = fullfile('ellip_pic_p_free', Dtype, "fullpara",CT_type,'theta', iOr);
            if ~exist(output_folder, 'dir')
                mkdir(output_folder);
            end
            ...
            % alpha 建模
            output_folder = fullfile('ellip_pic_p_free', Dtype, "fullpara",CT_type,'alpha', iOr); % 保存到 'alpha' 子文件夹
            if ~exist(output_folder, 'dir')
                mkdir(output_folder);
            end
