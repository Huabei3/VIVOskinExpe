        ...
        for idx_attribute = 1:length(attributes) % 使用新的索引来遍历 attributes 数组
            ...
            for i_nation = 1:length(nations)
                ...
                if idx_attribute == 7
                    ...
                    CT_interpolation_data{ i_nation, idx_attribute}.a_CL_all_CT(i_CT,:) = CT_old_data{ i_nation, idx_attribute}.a_CL_all_CT(i_CT,:);
                    CT_interpolation_data{ i_nation, idx_attribute}.a_hue_angle_all_CT(i_CT,:) = CT_old_data{ i_nation, idx_attribute}.a_hue_angle_all_CT(i_CT,:);
                    % 开关关闭时，将未拟合的参数设置为 NaN 或零
                    r_CL_all(i_nation) = NaN;
                    rmse_CL_all(i_nation) = NaN;
                    a_CL_all(i_nation, :) = NaN(1, 2);

                    r_hue_angle_all(i_nation) = NaN;
                    rmse_hue_angle_all(i_nation) = NaN;
                    a_hue_angle_all(i_nation, :) =  NaN(1, 2);
...
CT_interpolation_folder= fullfile("AnalyseResults_p_free",Dtype,scale_type_origin,"model_fullpara");

save(fullfile(CT_interpolation_folder, strcat('CT_interpolation.mat')), ...
    ...
    for i_nation = 1:length(nations)
        ...
        for idx_attribute = 1:length(attributes)
            ...
            save(fullfile(output_folder_target, 'CT_interpolation_params.mat'), ...
                'CT_interp_data', 'CT_types', 'nations', 'attribute', 'obs_type', 'fit_all_params_switch');
        end
    end
end
