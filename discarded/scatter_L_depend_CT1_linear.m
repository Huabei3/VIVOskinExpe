        ...
        for i_nation_load = 1:length(nations)
            ...
            for i_subject = 1:n_subjects
                ...
                for i_attr = 1:length(attributes)
                    ...
                    source_file = fullfile('AnalyseResults_p', Dtype,scale_type_origin, lastPart, obs_type_used, attribute_serial, 'ellipPara', 'fitRes.mat');
                    % 加载数据
                    if i_attr==7
                        disp("d")
                    end
        ...
        for idx_attribute = 1:length(attributes) % 使用新的索引来遍历 attributes 数组
            ...
            for i_nation = 1:length(nations)
                ...
                if idx_attribute == 7
                    ...
                    % C_L 建模 (总是执行)
                    output_folder_CL = fullfile('ellip_pic_p', Dtype, "fullpara",CT_type,'C_L', iOr);
                    ...
                    % 色调角建模 (总是执行)
                    output_folder_hue = fullfile('ellip_pic_p', Dtype, "fullpara",CT_type,'hue_angle', iOr);
                    ...
                    if fit_all_params_switch
                        % 长轴建模
                        output_folder_long = fullfile('ellip_pic_p', Dtype, "fullpara",CT_type,'long_axis', iOr);
                        ...
                        % 短轴建模
                        output_folder_short = fullfile('ellip_pic_p', Dtype, "fullpara",CT_type,'short_axis', iOr);
                        ...
                        % 椭圆倾角 theta 建模
                        output_folder_theta = fullfile('ellip_pic_p', Dtype, "fullpara",CT_type,'theta', iOr);
                        ...
                        % alpha 建模
                        output_folder_alpha = fullfile('ellip_pic_p', Dtype, "fullpara",CT_type,'alpha', iOr); % 保存到 'alpha' 子文件夹
...
CT_interpolation_folder= fullfile("AnalyseResults_p",Dtype,scale_type_origin,"model_fullpara");
...
% -------------------------------------------------------------------------
output_folder_params_base = fullfile('AnalyseResults_p', Dtype, scale_type_origin, "model_CT_interp", iOr);
