...
%% 设置拟合开关
% true: 拟合所有参数 (C_L, long_axis, short_axis, hue_angle, theta, alpha)
% false: 只拟合 C_L 和 hue_angle
    ...
    for i_obs = 1:length(obs_types_process)
        ...
        for idx_attribute = 1:length(attributes) % 使用新的索引来遍历 attributes 数组
            ...
            if use_old_fit && ~re_fit
                params_source_folder = fullfile('AnalyseResults_p_free', Dtype, scale_type_origin, 'model_fullpara', CT_type, iOr, obs_type);
                params_source_file = fullfile(params_source_folder, strcat(attribute_serial, '_all_curve_params.mat'));
                if exist(params_source_file, 'file')
                    params_cache = load(params_source_file);
                else
            ...
            for i_nation = 1:length(nations)
                ...
                if idx_attribute == 7
                    ...
                    if use_new_fit
                        [r_C_L, a_C_L, RMSE_C_L] = model_C_L_linear(output_folder_CL,L_all, C_all, attribute_serial, colors(i_nation, :), line_styles{line_style_idx}, Dtype, iOr, i_nation);
                    else
                        [r_C_L, a_C_L, RMSE_C_L] = model_C_L(output_folder_CL,L_all, C_all, attribute_serial, colors(i_nation, :), line_styles{line_style_idx}, Dtype, iOr, i_nation);
                    end
                    ...
                    if use_new_fit
                        hue_avg = NaN;
                        hue_valid = hue_angle(~isnan(hue_angle));
                        if ~isempty(hue_valid)
                            hue_avg = mean(hue_valid);
                        end
                        r_hue_angle = NaN;
                    ...
                    if ~exist(output_folder_hue, 'dir')
                        mkdir(output_folder_hue);
                    end
                    if use_old_fit && ~re_fit
                        ...
                        [r_hue_angle, a_hue_angle, RMSE_angle] = model_hue_angle(output_folder_hue,L_all, hue_angle, attribute_serial, colors(i_nation, :), line_styles{line_style_idx}, Dtype, iOr, i_nation);
                    end
                    end
            ...
            r_excel_output_folder = fullfile("AnalyseResults_p_free",Dtype, ...
                scale_type_origin, ...
                "model_fullpara",CT_type,fit_type, iOr, obs_type);
            if ~exist(r_excel_output_folder, 'dir')
                mkdir(r_excel_output_folder);
            end
            save(fullfile(r_excel_output_folder, strcat(attribute_serial,'.mat')), ...
            'a_alpha_all',"r_alpha_all","rmse_hue_angle_all", ...
        ...
        writematrix([full_header1; [cellstr(attribute_names_new(attributes))', size(num2cell(a_for_excel_C_L))]], excel_filename_C_L, 'Sheet', 'a');
