...
%--------model-group-------------
% source_folder = 'D:\work\VIVOskinExpe\analyze\expRes\malevivoi\model_group'; 
% group_type=2;
%--------model-------------
% source_folder = 'D:\work\VIVOskinExpe\analyze\expRes\malevivoi\model'; 
% group_type=3;
% %--------model-------------
% source_folder = 'D:\work\VIVOskinExpe\analyze\expRes\malevivoi\all'; 
% group_type=4;
%--------all_ruddy_normal-------------
% source_folder = 'D:\work\VIVOskinExpe\analyze\expRes\female78i\all_ruddy_normal'; 
% group_type=5;
%------------------------
ifCAT=1;Dtype='summer';
...
for attribute = attributes
    ...
    if strcmp(data_type,"Asian_ruddy")
        ...
        source_folder_used=source_folder1;
    else
        source_folder_used=source_folder;
    end
    ...
    % 处理数据并保存
    if ifCAT
        process_data_CAT(dir_res, output_folder, lastPart,Dtype,data_type);
    else
    ...
    for i_group = 1:length(dir_labNgroup)
        ...
        % elseif min(parData.par_all(i_group,4:5))>7
        %     par=parData.par_all(i_group,:);
        %     r=parData.r_all(i_group,:);
        %     row_delete=deleteData.row_delete_all{i_group,1}{1,1};
        %     row_delete_lab_group=deleteData.row_delete_all{i_group,1}{1,2};
