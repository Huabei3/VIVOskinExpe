...
%     xyz_fit(i_level,:)=lab2xyz2(Lab_cen(i_level,:),"d65_64");
%     lab_scaled(i_level,:)=xyz2lab(xyz_fit(i_level,:),"user", ...
%         wd65_64./wd65_64(2).*Y_temp);    
% end
% L=lab_scaled(:,1);
%% C=a1*log(L)+a2
...
for i_para=1:n_para
    ...
    % text( C_all(i_para),L(i_para), strcat('{\it L*=}',num2str(i_para),'0'), ...
...
%设置坐标
ax = gca; ax.XLim = [0 max(C_all)+5];
ay = gca; ay.YLim = [0 max(L)+5];
xlabel('C_{ab}*','FontAngle', 'italic');
ylabel('L*','FontAngle','italic');

title('C_{ab}*-L*','FontAngle', 'italic');
saveas(4,'ellip_pic\linear\m\C_L_curve.jpg');
