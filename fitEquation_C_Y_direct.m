...
xdata=xdata';
% xdata1=100*((xdata+16)/116).^3;
xdata1=((xdata+16)/116).^3;
ydata = C;
ydata=ydata;

f = @(a,xdata1)(a(1)*log10(xdata1)+a(2));
...
for t = 1:500
    ...
    a = lsqcurvefit(f,a0,xdata1,ydata,[-inf,-inf],[inf,inf],options);
    y = a(1)*log10(xdata1)+a(2);
...
scatter(xdata1,ydata, 40, 'filled'); 
hold on;
x = 0:0.01:60;
y= a(1)*log10(x)+a(2);
text(55,5,strcat('C=',num2str(a(1)),'*log10(Y)+',num2str(a(2))), ...
...
saveas(1,['E:\documents\MATLAB\SkinColorPreferenceScale\ellip_pic' ...
    '\equation1_Y_direct.jpg']);
%%
%equation2
% xdata1=[xdata1;0.0001];
% ydata=[ydata;0.0001];
f = @(a,xdata1)(a(1)*log10(xdata1)+ ...
    a(2)*log10(xdata1).^2+a(3));
...
for t = 1:500
    a0 = [rand,rand,rand];
    options = optimset('MaxFunEvals',200000);
    a = lsqcurvefit(f,a0,xdata1,ydata,[-inf,-inf,-inf],[inf,inf,inf],options);
    y = a(1)*log10(xdata1)+a(2)*log10(xdata1).^2+a(3);
...
hold on;
x = 0:0.1:60;
y= a(1)*log10(x)+a(2)*log10(x).^2+a(3);
text( 35,2,{'C=',num2str(a(1)),'*log10(Y)+', ...
    num2str(a(2)),'*log10(Y)^2+', ...
    num2str(a(3))}, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'FontSize', 10);
text( 35,15,strcat('r=',num2str(r2)), 'VerticalAlignment', ...
    'bottom', 'HorizontalAlignment', 'right', 'FontSize', 10);
plot(x,y);
