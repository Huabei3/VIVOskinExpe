...
attribute_names = ["Preference", "Attractiveness", "Feminine", "Cooperative", ...
    "Youth", "Healthy", "Precise reproduction","suit the environment or not", "white-skinned", "ruddy", "Precise reproduction"];

%% 定义 lastPart 文件夹路径
lastPart = 'malevivoi'; % 替换为实际的文件夹名称
source_folder = fullfile('ellip_pic\ellipse', lastPart); % 替换为实际的路径

%% 创建保存大图的文件夹
output_folder = fullfile(source_folder, 'concatenated_images');
...
for attribute = attributes
    ...
    % 获取当前 attribute_serial 文件夹路径
    attribute_folder = fullfile(source_folder, attribute_serial);
    
    % 检查文件夹是否存在
    if ~exist(attribute_folder, 'dir')
    ...
    % 调用拼接函数
    try
        concatenate_images(attribute_folder, attribute_serial, output_folder);
    catch ME
        fprintf('Error processing attribute %d: %s\n', attribute, ME.message);
    end
end

disp('All attributes processed successfully.');

%% 拼接函数
function concatenate_images(save_folder, attribute_serial, output_folder)
    % 读取输入目录中的所有JPG图片文件
