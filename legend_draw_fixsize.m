function legend_draw_size( output_folder,legend_type)
    % 输入参数：
    % tables - 字符串数组，包含图例文本
    ...
    % Calculate target resolution for 15811px width
    screen_ppi = get(0, 'ScreenPixelsPerInch');
    target_width_px = 15811;
    fig_width_pixels = 1200; % From Position
    fig_width_inch = fig_width_pixels / screen_ppi;
    target_resolution = round(target_width_px / fig_width_inch);
    text_size = 15; % 1/60 of width (12.5 inch * 72 pt/inch * 1/60 = 15 pt)
    hold on;
...
%%
    ...
    % ===================== 步骤2：获取exportgraphics裁剪后的图片实际尺寸 =====================
    % 先导出临时图获取实际尺寸（无视觉差异，仅用于获取尺寸）
    temp_file = [tempname '.png'];
    exportgraphics(gcf, temp_file, 'Resolution', target_resolution, 'ContentType', 'image');
    img_temp = imread(temp_file);
    delete(temp_file); % 删除临时文件
    img_width_px = size(img_temp, 2);  % 裁剪后图片宽度（像素）
    img_height_px = size(img_temp, 1); % 裁剪后图片高度（像素）
    img_resolution = 150; % 导出分辨率（与exportgraphics一致）

    % ===================== 步骤3：计算文本缩放系数 =====================
    current_text_size = 15; % 图例当前文本字号（固定值）
    % 缩放系数 = 目标字号 / 当前字号（保证缩放后与主图title_font_size一致）
    scale_factor = title_font_size_target / current_text_size;

    % ===================== 步骤4：保存mat文件 =====================
    output_file = fullfile(output_folder, strcat("z_",legend_type,'.jpg'));
    % mat文件命名：与jpg文件同名，后缀改为.mat
    mat_file = strrep(output_file, '.jpg', '.mat');
    % 保存的变量：图片尺寸、分辨率、缩放系数、目标字号
    save(mat_file, ...
        'img_width_px', 'img_height_px', 'img_resolution', ...
        'scale_factor', 'title_font_size_target', 'current_text_size');


    % 保存为 JPG 文件
    exportgraphics(gcf, output_file, 'Resolution', target_resolution, 'ContentType', 'image');
...
legend_draw_size(output_folder,legend_type);
%%
% Dtype = "efit_p";
% output_folder = fullfile('ellip_pic_p', Dtype, 'legend');
% if ~exist(output_folder, 'dir')
...
% 
%     legend_draw_size1(output_folder,nation);
% end
