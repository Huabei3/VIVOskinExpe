function legend_draw_size( output_folder,legend_type)
    ...
    h1=figure('Position', [0 0 1200 200]); % 设置图形窗口大小
    
    % Calculate parameters
    screen_ppi = get(0, 'ScreenPixelsPerInch');
    fig_width_pixels = 1200; % From Position
    
    % Calculate font size in points to achieve 1/60 ratio
    % text_size = (fig_width_pixels * text_ratio) * (72 / screen_ppi);
    text_size = (1200 * text_ratio) * (72 / screen_ppi);
...
%%
    ...
    % 使用 print 替代 exportgraphics 以保留背景和尺寸
    % 计算尺寸和分辨率
    img_width_px = target_width_px;
    img_height_px = round(target_width_px * (200/1200)); % 保持比例
    img_resolution = target_resolution;
    
    scale_factor = 1;
    title_font_size_target = text_size * (target_width_px/1200); % 逻辑上的一致性检查，实际 text_size 已定
    ...
    set(gcf, 'PaperPosition', [0 0 1200/screen_ppi 200/screen_ppi]);
    set(gcf, 'PaperSize', [1200/screen_ppi 200/screen_ppi]);
    
    % 打印保存
    print(gcf, output_file, '-djpeg', ['-r', num2str(target_resolution)]);
