...
% 关键开关（可按需改）
if_fit              = "false";       % "true": 包含椭圆拟合结果(par,lab_center), "false": 不拟合，table中无par和lab_center字段
            ...
            for i_subject = 1:n_subjects
                ...
                for i_attr = 1:numel(attributes)
                    ...
                    if strcmp(if_fit, "true")
                        ...
                        % if_fit="false": 不加载 ellipPara，par_all 置空
                        par_all = [];
                        % 确定 n_row（需要知道有多少个 i_par）
                        % 通过 labNscore 文件数量来确定
                        dir_labN = dir(fullfile(base_analyze,'AnalyseResults_p', Dtype, scale_type_origin, ...
                            lastPart, obs_type, attribute_serial, 'labNscore', ...
                            strcat('labNscore_group',lastPart, picnames_groups(1),'*.mat')));
                        if isempty(dir_labN)
                            continue;
                        end
                    end
                    ...
                    if strcmp(if_fit, "true")
                        n_par_loop = size(par_all,1);
                    else
                        n_par_loop = numel(picnames_groups);
                    end
                    for i_par=1:n_par_loop
...
if strcmp(if_fit, "true")
    ...
    fit_table = table( ...
        col_source, col_lab_values, col_opinion_scores, col_fit_equation, ...
        col_average_lab, col_cct, col_illuminance, col_scene, col_nodel_lighting, ...
        col_D_equation_str, col_model_ethnicity, col_model_gender, col_model_id, col_observer_type, ...
        col_opinion_score_range, col_other_info,col_attr, ...
        'VariableNames', { ...
            'source', 'lab_values', 'opinion_scores', 'fit_equation', ...
            'average_lab', 'CCT', 'illuminance', 'scene', 'lighting', ...
            'D_equation_str', 'model_ethnicity', 'gender', 'model_id', 'observer_type', ...
            'opinion_score_range', 'other_info','attribute' ...
        } ...
    );
...
if strcmp(if_fit, "true")
    ...
    output_folder = fullfile("AnalyseResults_p",Dtype, ...
        scale_type,"no_fit","resTable",strcat("original_",scale_type_origin));
end
...
excel_table = summarize_for_excel_final(fit_table, if_fit);
writetable(excel_table, out_xlsx, 'FileType', 'spreadsheet');

fprintf('Saved: %s\n', fullfile(pwd,out_mat));
fprintf('Saved: %s\n', out_xlsx);

% main

function T = summarize_for_excel_final(fit_table, if_fit)
