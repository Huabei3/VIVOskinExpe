...
for i_obs = 1:length(obs_types)
    ...
    for i_nation = 1:length(nations)
        ...
        for attribute = attributes
            ...
            fprintf('--- Processing: ObsType=%s, Nation=%s,  Attribute=%s ---\n', obs_type, nation, attribute_names_new(attribute));
            
            % 定义文件路径
            source_folder = fullfile('AnalyseResults1',Dtype,'50',"nation",obs_type, ...
            ...
            if ~found_valid_data || all(cellfun(@isempty, all_par))
                fprintf('没有为 ObsType=%s, Nation=%s,  Attribute=%s 找到有效数据。跳过。\n', obs_type, nation, attribute_names_new(attribute));
                continue;
            end
            ...
            if sum(valid_indices) >= 2 
                ...
                save_folder_fit_params = fullfile('AnalyseResults1', Dtype,"nation" ,'scale_factor_fit_results', obs_type,iOr, nation);
                ...
                % 绘制拟合曲线图
                h_fig_fit = figure('Name', sprintf('Scale Factor Fit for %s-%s-%s-%s', obs_type, nation, attribute_names_new(attribute)));
                ...
                ylabel('Scale Factor');
                title(sprintf('Scale Factor vs. L* for %s %s %s (%s)', obs_type, nation, attribute_names_new(attribute)));
                legend('Location', 'best');
                grid on;
                save_folder_fit_curves = fullfile('AnalyseResults1', Dtype,"nation", 'scale_factor_curves', obs_type, iOr,nation);
                if ~exist(save_folder_fit_curves, 'dir')
                    mkdir(save_folder_fit_curves);
                end
                saveas(h_fig_fit, fullfile(save_folder_fit_curves, sprintf('ScaleFactor_Curve_%s_%s_%s_%s.png', obs_type, nation, attribute_serial)));
                close(h_fig_fit); % 关闭图窗

                % --- 绘制最终等高线图 (使用 get_par_fr_SF 逻辑) ---
                h_fig_contour_get_par_fr_SF = figure('Name', sprintf('Contours (get_par_fr_SF Logic) for %s-%s-%s', obs_type, nation, attribute_names_new(attribute)));
                hold on;
                grid on;
                axis equal;
                ...
                xlabel('a*');
                ylabel('b*');
                title(sprintf('Contours (get_par_fr_SF Logic) for %s %s %s (%s)', obs_type, nation, attribute_names_new(attribute)));
                ...
                save_folder_contour_get_par_fr_SF = fullfile('AnalyseResults1', ...
                    ...
                    nation, attribute_serial);
                if ~exist(save_folder_contour_get_par_fr_SF, 'dir')
                    mkdir(save_folder_contour_get_par_fr_SF);
                end
                saveas(h_fig_contour_get_par_fr_SF, fullfile(save_folder_contour_get_par_fr_SF, sprintf('Contours_get_par_fr_SF_%s_%s_%s_%s.png', obs_type, nation, attribute_serial)));
