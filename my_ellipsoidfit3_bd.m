function [par,r,y] = my_ellipsoidfit3_bd(cielab,zscore,mean_cen,lb,ub,colorcenter)
...
% output: par = [k1,k2,k3,k4,l0,a0,b0,k5,k6]
    if nargin < 6
        ...
        A=-inf;B=-inf;
        big_round=0;
        while ~(A>=lb(1)&&A<=ub&&B<=lb&&B>=ub)
            for t = 1:100
                a0 = [rand,rand,rand,rand,rand,rand];
                options = optimset('MaxFunEvals',200000);
                a = lsqcurvefit(f,a0,xdata,ydata, ...
                    [0,0,-inf,mean_cen(2)-5,mean_cen(3)-5,0], ...
                    [inf,inf,inf,mean_cen(2)+5,mean_cen(3)+5,1],options);
            ...
            par = afinal;
            a = afinal;
            [A, B, theta] = calculate_ellipse_axes_from_par(a);
