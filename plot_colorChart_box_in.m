...
function export_outliers(~, ~, fig_handle, plot_type, all_plot_data)
    ...
    output_folder = fullfile("AnalyseResults1", "efit2", "inliers");
    if ~exist(output_folder, "dir")
        mkdir(output_folder);
    end
    ...
    if strcmp(plot_type, 'ab')
        ...
        filename = fullfile(output_folder, 'inliers_ab_plot.xlsx');
        coord_names = {'a*', 'b*'};
    else % 'LC'
        x_coords = [all_plot_data.C];
        y_coords = [all_plot_data.L];
        filename = fullfile(output_folder, 'inliers_LC_plot.xlsx');
        coord_names = {'C*', 'L*'};
    end
    
    % ⚠️ Modified part: Delete the file if it exists
    if exist(filename, 'file')
        delete(filename);
    end
    
    % Find points inside the selected rectangle
    inside_indices = find(x_coords >= x_min & x_coords <= x_max & y_coords >= y_min & y_coords <= y_max);
    
    if isempty(inside_indices)
        disp('No points found inside the selected region.');
        return;
    end
    ...
    % Loop through the inside points and get their details
    for i = 1:length(inside_indices)
        idx = inside_indices(i);
        ...
        new_row = table(...
            ...
            'VariableNames', {'Picname', 'Attribute', 'LastPart', 'Coordinates'});
        
        inliers_table = [inliers_table; new_row];
    end
    
    % 1. Remove rows with NaN values before saving
    nan_rows = any(isnan(table2array(inliers_table(:, {'Coordinates'}))), 2);
    inliers_table(nan_rows, :) = [];
    ...
    writetable(inliers_table, filename, 'Sheet', 'Inliers');
    writetable(rect_table, filename, 'Sheet', 'Rectangle Coordinates');
    
    disp(['Inliers and rectangle coordinates written to: ' filename]);
end
