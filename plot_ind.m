...
%1 2 4
%--------non-model-group-------------
source_folder = 'D:\work\VIVOskinExpe\analyze\expRes\f05r\renamed\non_model'; 
group_type=1;
...
for attribute = attributes
    ...
    for i_obs=1:length(dir_res)
        ...
        for i_group = 1:length(lab_group_sing)
            ...
            output_folder_group = fullfile(output_folder,...
                gen_lastPart_new(picname_check{i_group, 1}));
            if ~exist(output_folder_group,"dir")
                mkdir(output_folder_group);
            end
            exportgraphics(gcf, fullfile(output_folder_group, ...
                strcat(lastPart, obs,'.jpg')),'Resolution',150);
            clf;
        end
