function [outnew,  out_of_gamut_ratio] = img_AddRender_dlab(img, bull, string,dlab)
...
%     datai_file = 'Z:\homes\Peggy\oppoSkinExperi\LUT3d\results\datai_sorted53_3OV.mat';
    ...
    mean_lab=mean(lab1);
    rela=lab1-repmat(mean_lab,length(lab1),1);
    lab2=dlab+rela;
...
%%
    ...
    elseif matrix==3
        ...
        if any(~logicalIndex)            
%             [rgbnew_bull1,out_of_gamut_ratio] = lut3d_xyz2rgbIpv_bfCashe(xyz2(~logicalIndex, :), datafile);
            [rgbnew_bull1,out_of_gamut_ratio] = lut3d_xyz2rgbKD(xyz2(~logicalIndex, :), datafile);
        else
            rgbnew_bull1 = [];
        end
