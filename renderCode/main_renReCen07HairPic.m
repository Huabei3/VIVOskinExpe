...
HairLinePic=[15,24,27];
% BigDeCenter=[6,8,9,11,15,16,21,24,27,31,33,34];
harLine=[25,29];

% for i = 33:33
for i = 25:numel(files)
    if ~ismember(i, HairLinePic)
        continue
    end    
    num_of_g=floor(i/10);
    ...
    for i_points=startCenter:endCenter
        ...
        img_file=['Z:\homes\Peggy\oppoSkinExperi\picked40\iphone\cropped\renderedLUTchoose\renderedReCen07\' ...
        ...
        imwrite(out_rendering,['Z:\homes\Peggy\oppoSkinExperi\picked40\iphone\cropped\renderedLUTchoose\renderedReCen07\' ...
            ,files(i).name(1:end-4),'_',sprintf('%02d', i_points),'[',num2str(dlab(1,1)),',' ,...
            num2str(dlab(1,2)),',',num2str(dlab(1,3)),'].jpg'] );
