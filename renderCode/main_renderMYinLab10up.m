...
for i = 1:numel(files)
    ...
    for i_points=startCenter:endCenter
        ...
        delta_Lab1=delta_Lab;
        if delta_Lab1(1,1)<=0
            continue
        end
        dlab=average(i,:)+delta_Lab1;

        img_file=['Z:\homes\Peggy\oppoSkinExperi\picked40\Hassel\downSampled1\cropped\renderedAdd\renderedAdd10up\' ...
        ...
        imwrite(out_rendering,['Z:\homes\Peggy\oppoSkinExperi\picked40\Hassel\downSampled1\cropped\renderedAdd\renderedAdd10up\' ...
            ,files(i).name(1:end-4),'_',sprintf('%02d', i_points),'[',num2str(dlab(1,1)),',' ,...
            num2str(dlab(1,2)),',',num2str(dlab(1,3)),'].jpg'] );
