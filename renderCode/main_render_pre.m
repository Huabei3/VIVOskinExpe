...
for i = 3:numel(files)
    ...
    load(strcat(dir_pointsfiles(i).folder,'\',dir_pointsfiles(i).name));%茅鈥斅伱モ?樑犆β⒙幻β柯ζ捖兟姑┞嵚Ｃ︹?β箃Points
    spq=1;
    ...
    for i_points=startCenter:endCenter
        ...
        [out_rendering,outxyz,k,out_awb,outofgamut]=imgcat2_1_preSkipNF(img,bull,predict_white,white65,white65,w,'LUT',i,i_points);
%         [out_rendering,outxyz,k,out_awb,outofgamut]=imgcat2_1(img,bull,predict_white,white65,white65,w,'LUT',i,i_points);
        ...
        if outofgamut<0.15
        imwrite(out_rendering,['Z:\homes\Peggy\oppoSkinExperi\picked40\iphone\cropped\renderedLUT06\' ...
...
%             num2str(dlab(1,2)),',',num2str(dlab(1,3)),'].jpg'] );
        ...
        imwrite(out_rendering,['Z:\homes\Peggy\oppoSkinExperi\picked40\iphone\cropped\renderedLUT06\outofgamut\' ...
            ,files(i).name(1:end-4),'_',sprintf('%02d', i_points),'[',num2str(dlab(1,1)),',' ,...
            num2str(dlab(1,2)),',',num2str(dlab(1,3)),'].jpg'] );
