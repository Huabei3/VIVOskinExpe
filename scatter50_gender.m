...
for i_obs = 1:length(obs_types)
    ...
    % for attribute = 1:length(attributes)
        ...
        % 为当前属性创建一个临时存储
        parNr_temp = zeros(n_para, 7, 2);  % 每个参数有7个值，两个性别
        
        for i_para =target_indices
            figure(attribute*10+i_para); hold on;
            % for i_nation = 4:4
            for i_nation = 1:length(nations)
                % 获取当前人种的所有索引
                nation = nations(i_nation);
                ...
                for i_gender = 1:2
                    ...
                    for i_subject = 1:length(curr_gender_indices)     
                        ...
                        lab_group = [lab_group; lab_curr(:,:,subject_idx)];
                        MSV_group = [MSV_group; MSV_curr(:,subject_idx,attribute)];

                        [par_mean_ind(i_subject,:), r_mean_ind(i_subject,:)] = ...
                            calculate_weighted_or_simple_mean...
                            (MSV_curr(:,subject_idx,attribute), lab_curr(:,:,subject_idx));
                    end
                    ...
                    if ~all(isnan(MSV_group(:)))
                        ...
                        lab_group_original=lab_group;
                        MSV_group_original=MSV_group;

                        if i_nation==4 && i_para==19
                            continue
                        end

                        while true
                            [par, r, y] = my_ellipsoidfit3_dy(lab_group, MSV_group, mean_cen);
                            if (4*par(1)*par(2)-par(3)^2)>0&&r>0.6
                                break;
                            end
                            % 找到最大误差的索引
                            ...
                            lab_group(max_ind, :) = [];
                            MSV_group(max_ind, :) = [];
                            figure(attribute*10+i_para);
                            plot_contour_with_scatter(par, lab_group, MSV_group);
                        end
                        
                        % 绘制等高线图并保存
                        pre_draw_folder = fullfile(save_folder, 'pre_draw',obs_type,iOr);
                        ...
                        for i_subject = 1:length(curr_gender_indices)
                            subject_idx = curr_nation_indices(i_subject);
                            lastPart = lastParts{subject_idx};
                            scatter(par_mean_ind(i_subject,4), par_mean_ind(i_subject,5), 20, ...
                                'p', 'MarkerFaceColor', colors(i_nation,:));
                            text(par_mean_ind(i_subject,4), par_mean_ind(i_subject,5),  char(lastPart), ...
                                'Color', colors(i_nation,:), 'FontSize', 4);
                        end
                        
                        plot_contour_with_scatter(par, lab_group, MSV_group);
                        saveas(12, fullfile(pre_draw_folder, ...
                            strcat(nation,gender,'.jpg')));
                        close(12);

                        parNr_temp(i_para,:,i_gender) = [par, r];
                        %保存ellipPara
        ...
        for i_nation = 1:length(nations)
            for i_gender = 1:2
                parNr_all{i_obs, i_nation, i_gender, attribute} = parNr_temp(:,:,i_gender);
            end
        end
        ...
        saveas(attribute*10+i_para, fullfile(output_folder, ...
            strcat(attribute_serial,".jpg")));
        close(attribute*10+i_para);  % 关闭当前图形
    end
