        ...
        for i_nation = 1:length(nations)           
            ...
            for i_subject = 1:n_subjects
                ...
                for attribute = 1:length(attributes)
                    ...
                    % 定义源文件路径
                    source_file = fullfile('AnalyseResults_p_free', Dtype, scale_type_origin,lastPart, ...
...
%% 保存数据
save_folder = fullfile("ellip_pic_p_free", Dtype,"50");
if ~exist(save_folder, "dir")
    mkdir(save_folder);
end
% 保存重塑后的数据
save(fullfile(save_folder, strcat("data_reshaped_", iOr, ".mat")), ...
     "lab_reshaped", "p_reshaped", "average_reshaped","average_nation");

fprintf('数据已成功保存到 %s\n', fullfile(save_folder, strcat("data_reshaped_", iOr, ".mat")));
%% 
% Dtype="efit_p_free";
% load(fullfile("ellip_pic_p_free",Dtype,"50", ...
...
cluster_samples{1}=[1,2,4,5,6];
cluster_samples{2}=[7,8,9,10];
cluster_samples{3}=[11,12];
cluster_samples{4}=[13,14];
    ...
    for i_nation = 1:length(nations)
        ...
        for attribute = 1:length(attributes)
            ...
            for i_cluster=1:size(cluster_samples,1) 
                ...
                for i_para = cluster_samples{i_cluster}
                    for i_subject = 1:n_subjects
                        subject_idx = curr_nation_indices(i_subject);
                        lastPart = lastParts{subject_idx};
                ...
                average_cluster{i_nation,i_cluster}=nanmean(average_cluster_curr,1);
                % 过滤掉p_group中全是NaN的行
                ...
                if ~all(isnan(p_group(:)))
                    ...
                    [par, r, y] = my_ellipsoidfit3_free(lab_group, p_group, mean_cen);
                    parNr(i_cluster,:) = [par, r];  % 移除性别维度
                    
                    Contour50(par, lab_group, p_group, "scene_50", ...
                    ...
                    fitRes_folder=fullfile("AnalyseResults_p_free",Dtype,scale_type_origin,"50","scene_50", ...
            ...
            output_folder = fullfile(save_folder,"scene_50",iOr,obs_type);
