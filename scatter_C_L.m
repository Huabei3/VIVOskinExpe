...
r_excel_output_folder = fullfile("AnalyseResults_p", Dtype, scale_type_origin,"sum_list", 'C_L');
...
for i_obs = 1:length(obs_types)
    ...
    % 用于存储当前 obs_type 下所有 attribute 的 r_CL_all 值，以便写入 Excel
    ...
    for idx_attribute = 1:length(attributes) % 使用新的索引来遍历 attributes 数组
        ...
        for i_nation = 1:length(nations)
            ...
            if ~isempty(lab_g_valid)
                ...
                if length(L_valid) > 3
                    ...
                    scatter(C_valid, L_valid, 8, colors(i_nation, :), 'o', 'filled', 'MarkerFaceAlpha', 0.6);
        ...
        % 将当前 attribute 的 r_CL_all 存入总的 r_values_for_excel 矩阵
        r_values_for_excel(idx_attribute, :) = r_CL_all'; % 转置以匹配行和列
        ...
        xlim([0, 35]);
        % 保存图片
        output_folder = fullfile('ellip_pic_p', Dtype, 'C_L', iOr, 'curve_fit');
        if ~exist(output_folder, 'dir')
            mkdir(output_folder);
        end
        ...
        % 保存拟合参数
        output_folder_params = fullfile('ellip_pic_p', Dtype, 'C_L', iOr, obs_type);
    ...
    excel_filename = fullfile(r_excel_output_folder, strcat('correlation_', iOr, '_', obs_type, '.xlsx'));

    % 构建表头
    attribute_names_cell = cellstr(attribute_names_new(attributes));
    header_row_left = "Attribute";
    
    % 使用 nations 作为 Excel 列名
    excel_col_names = nations; 

    % 将表头和数据组合
    data_to_write = [cellstr(attribute_names_new(attributes))', num2cell(r_values_for_excel)];

    % 创建一个包含所有列名的元胞数组，以便writematrix正确处理
    full_header = [cellstr(header_row_left), cellstr(excel_col_names)];
    
    % 写入 Excel 文件
    writecell([full_header; data_to_write], excel_filename, 'WriteMode', 'overwrite');
    fprintf('已将 %s 的相关性矩阵写入 Excel 文件。\n', obs_type);
    % --- Excel 写入结束 ---

    output_folder = fullfile("ellip_pic_p", Dtype, "C_L", iOr, "curve_fit");
    concatenate_images1(output_folder, 5); % 假设 concatenate_images1 函数已定义
end