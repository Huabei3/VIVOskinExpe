...
%   ③ [新增] 支持 iOr={'i','r'} 双轮运行，合并输出6个sheet的xlsx
close all; clc; clear;
addpath("utils\");

%% ===== 用户可调参数 =====
n_drop = 1;
if_draw = "false";   % "true"=出图, "false"=只算数据不出图
...
%% ========== 主循环：遍历 iOr = {'i', 'r'} ==========
all_results_by_iOr = cell(length(iOr_list), 1);
excel_filenames = cell(length(iOr_list), 1);

for iOr_loop_idx = 1:length(iOr_list)
    iOr = iOr_list{iOr_loop_idx};
    fprintf('\n======================================================\n');
    fprintf('========== 开始处理 iOr = "%s" (%d/%d) ==========\n', iOr, iOr_loop_idx, length(iOr_list));
    fprintf('======================================================\n');
    ...
    excel_filenames{iOr_loop_idx} = excel_filename;

    drop_colors = lines(20);
    all_results = struct();

    % ---- 初始化 xlsx sheet header ----
    if exist(excel_filename, 'file'), delete(excel_filename); end
    ...
    for i_n = 1:length(nations)
        sheet_name = strrep(nations(i_n), ' ', '_');
    ...
    for i_nation = 1:length(nations)
        ...
        for val_combo_idx = 1:n_subjects_nation
            ...
            for idx_attribute = 1:length(attributes)
                ...
                if attribute == 7, i_obs_used = 2; else, i_obs_used = 1; end
                ...
                if isempty(L_train) || isempty(par_train_valid)
                    C_train=[]; hue_train=[]; alpha_tr=[];
                    lambda00_tr=[]; lambda01_tr=[]; lambda11_tr=[];
                    theta_tr=[]; A_tr=[]; B_tr=[];
                    long_axis_tr=[]; short_axis_tr=[];
                else
                    C_train     = sqrt(par_train_valid(:,4).^2 + par_train_valid(:,5).^2);
                    hue_train    = atan2d(par_train_valid(:,5), par_train_valid(:,4));
                    valid_alpha_idx = ~isnan(par_train_valid(:,6)) & par_train_valid(:,6) > 0;
                    alpha_tr = nan(size(par_train_valid(:,6)));
                    alpha_tr(valid_alpha_idx) = -log(par_train_valid(valid_alpha_idx,6));
                    ...
                    theta_tr    = mod(0.5 * atan2d(2*lambda01_tr, (lambda00_tr - lambda11_tr)), 360);
                ...
                % ===== 加载 fullpara 参数（[修改] 用变量 iOr 替代硬编码 'i'）=====
                model_fp_file = fullfile("D:\work\VIVOskinExpe\analyze\AnalyseResults_p", ...
                    "efit_p\unscaled\model_fullpara\d65", "new", iOr, char(obs_type_used), ...
                ...
                for i_tgt = 1:length(indices_target)
                    ...
                    if ~any(isnan(fp_a_CL_all)) && ~any(isnan(fp_a_long_axis_all))
                        ...
                        if ~isnan(fp_chroma) && ~isnan(fp_long_m) && ~isnan(fp_short_m) ...
                                && ~isnan(fp_a_alpha_all(1)) && fp_long_m > 0 && fp_short_m > 0
                            par_fullpara = calculate_par_from_ellipse(fp_a_hue_angle_all(1), ...
                                fp_chroma, fp_long_m, fp_short_m, fp_a_theta_all(1), fp_a_alpha_all(1));
                        end
                    end

                    % --- 加载 dropped subject 原椭圆 ---
                    ...
                    if ~isempty(par_exp) && length(par_exp) >= 6
                        exp_C_list = [exp_C_list; sqrt(par_exp(4)^2 + par_exp(5)^2)];
                    end
                    ...
                    % --- 加载 undropped subjects 的 lab/p_group ---
                    ...
                    for i_undrop = 1:length(train_indices)
                        ...
                        if exist(labNscore_undrop, 'file')
                            ...
                            if isfield(ld_u,'lab_group') && isfield(ld_u,'p_group') ...
                                ...
                                lab_undrop_all = [lab_undrop_all; ld_u.lab_group];
                                p_undrop_all   = [p_undrop_all;   ld_u.p_group];
                            end
                        end
                    end

                    % --- delta 计算 ---
                    delta_a4=NaN; delta_a5=NaN; delta_C=NaN; delta_la=NaN;
                    delta_sa=NaN; delta_hue=NaN; delta_theta=NaN; delta_alpha=NaN;
                    if ~isempty(par_ell) && ~isempty(par_fullpara) && length(par_ell)>=6 && length(par_fullpara)>=6
                        ...
                        la_ell = a_long_axis(1)*L_val_model^3+a_long_axis(2)*L_val_model^2+a_long_axis(3)*L_val_model+a_long_axis(4);
                        la_fp  = fp_a_long_axis_all(1)*L_val_model^3+fp_a_long_axis_all(2)*L_val_model^2+fp_a_long_axis_all(3)*L_val_model+fp_a_long_axis_all(4);
                        sa_ell = a_short_axis(1)*L_val_model^3+a_short_axis(2)*L_val_model^2+a_short_axis(3)*L_val_model+a_short_axis(4);
                        sa_fp  = fp_a_short_axis_all(1)*L_val_model^3+fp_a_short_axis_all(2)*L_val_model^2+fp_a_short_axis_all(3)*L_val_model+fp_a_short_axis_all(4);
                        delta_C = C_ell-C_fp; delta_la=la_ell-la_fp; delta_sa=sa_ell-sa_fp;
                    ...
                    ipr = struct();
                    ipr.i_par=i_par; ipr.pcn_name=current_pcn_name;
                    ipr.lab_group=lab_group_tgt; ipr.p_group=p_group_tgt;
                    ipr.lab_undrop=lab_undrop_all; ipr.p_undrop=p_undrop_all;
                    ipr.par_ell=par_ell; ipr.par_exp=par_exp;
                    ipr.par_fullpara=par_fullpara; ipr.L_val_model=L_val_model;
                    ipr.r_y=NaN; ipr.rmse_y=NaN;
                ...
                if length(model_C_list)>=3 && std(model_C_list)>0 && length(exp_C_list)==length(model_C_list) && std(exp_C_list)>0
                ...
                if length(exp_C_list)>=3 && std(exp_C_list)>0
                    ...
                    mask_exp = ~isnan(L_exp_v) & ~isnan(exp_C_list);
                    if sum(mask_exp)>=3 && std(L_exp_v(mask_exp))>0
                        r_C_exp = corr(L_exp_v(mask_exp)', exp_C_list(mask_exp)');
                    else
                        r_C_exp = NaN;
                    end
                else
                ...
                d_a4=[]; d_a5=[]; d_C=[]; d_la=[]; d_sa=[];
                d_hue=[]; d_th=[]; d_al=[];
                for ii = 1:length(fn_pcn_avg)
                    ipr_tmp = i_par_results.(fn_pcn_avg{ii});
                    d_a4=[d_a4;ipr_tmp.delta_a4]; d_a5=[d_a5;ipr_tmp.delta_a5];
                    d_C=[d_C;ipr_tmp.delta_C]; d_la=[d_la;ipr_tmp.delta_la];
                    d_sa=[d_sa;ipr_tmp.delta_sa]; d_hue=[d_hue;ipr_tmp.delta_hue];
                    d_th=[d_th;ipr_tmp.delta_theta]; d_al=[d_al;ipr_tmp.delta_alpha];
                end
                ...
                % ===== .mat 行 =====
                mat_row = struct();
                mat_row.attr_name=attr_name; mat_row.dropped_lastPart=dropped_lastPart;
                mat_row.dropped_subject_idx=dropped_subject_idx;
                mat_row.r_model_y=r_model_y; mat_row.rmse_y=rmse_model_y;
                mat_row.r_C_model=r_C_model; mat_row.r_C_exp=r_C_exp;
                mat_row.a_C_L=a_C_L; mat_row.a_long_axis=a_long_axis;
                mat_row.a_short_axis=a_short_axis; mat_row.a_hue_angle=a_hue_angle;
                mat_row.a_theta=a_theta; mat_row.a_alpha=a_alpha;
                mat_row.fp_a_CL=fp_a_CL_all; mat_row.fp_a_long_axis=fp_a_long_axis_all;
                mat_row.fp_a_short_axis=fp_a_short_axis_all; mat_row.fp_a_hue_angle=fp_a_hue_angle_all;
                mat_row.fp_theta=fp_a_theta_all; mat_row.fp_alpha=fp_a_alpha_all;
                ...
                mat_row.i_par_results=i_par_results;
                val_result.attr_results{end+1} = mat_row;

                % ===== 绘图 =====
                if strcmpi(if_draw, "true")
                    fn_pcn = fieldnames(i_par_results);
                    for ip_fig = 1:length(fn_pcn)
                        ipr_fig = i_par_results.(fn_pcn{ip_fig});
                        plot_n_drop_scatter_contour(ipr_fig, nation_serial, dropped_lastPart, ...
                            attr_name, fn_pcn{ip_fig}, train_indices, pic_folder);
                    end
                end

            end  % end attribute

            val_result.r_model_y_summary = r_model_y;
            nation_result.val_results{end+1} = val_result;
    ...
    all_results_by_iOr{iOr_loop_idx} = all_results;
    fprintf('========== iOr = "%s" 处理完成 ==========\n', iOr);
end  % end iOr loop
...
%% ========== 合并输出：生成6个sheet的总表 ==========
fprintf('\n\n======================================================\n');
fprintf('========== 生成合并输出 xlsx (6 sheets) ==========\n');
fprintf('======================================================\n');

combined_dir = fullfile("AnalyseResults_p", Dtype, scale_type_origin, ...
    "model_fullpara_n_drop", CT_type, "new", "combined", obs_folder_name, drop_folder_name);
if ~exist(combined_dir, 'dir'), mkdir(combined_dir); end
combined_xlsx = fullfile(combined_dir, sprintf('n_drop_cv_results_n%d_combined.xlsx', n_drop));
if exist(combined_xlsx, 'file'), delete(combined_xlsx); end

col_hdr = {'Attr', 'Drop_subj', 'Drop_LP', 'r_model_y', 'rmse_y', 'r_C_mod', 'r_C_exp', ...
    'a_C1', 'a_C2', 'a_la1', 'a_la2', 'a_la3', 'a_la4', ...
    'a_sa1', 'a_sa2', 'a_sa3', 'a_sa4', 'hue_angle', 'theta', 'alpha', ...
    'fp_a_C1', 'fp_a_C2', 'fp_a_la1', 'fp_a_la2', 'fp_a_la3', 'fp_a_la4', ...
    'fp_a_sa1', 'fp_a_sa2', 'fp_a_sa3', 'fp_a_sa4', ...
    'fp_hue_angle', 'fp_theta', 'fp_alpha', ...
    'delta_a4', 'delta_a5', 'delta_C', 'delta_la', 'delta_sa', ...
    'delta_hue', 'delta_theta', 'delta_alpha'};

% ---- Sheet 1: i_only ----
writecell(col_hdr, combined_xlsx, 'Sheet', 'i_only', 'WriteMode', 'overwritesheet');
for i_n = 1:length(nations)-1
    sn = strrep(nations(i_n),' ','_');
    try
        [~,~,sd] = xlsread(excel_filenames{1}, sn);
        if size(sd,1)>1
            for rr=2:size(sd,1), writecell(num2cell(sd(rr,:))', combined_xlsx, 'Sheet','i_only','WriteMode','append'); end
        end
    catch, fprintf('  [警告] 无法读取 %s sheet %s\n', excel_filenames{1}, sn); end
end
fprintf('  Sheet "i_only" OK\n');

% ---- Sheet 2: r_only ----
writecell(col_hdr, combined_xlsx, 'Sheet', 'r_only', 'WriteMode', 'overwritesheet');
for i_n = 1:length(nations)-1
    sn = strrep(nations(i_n),' ','_');
    try
        [~,~,sd] = xlsread(excel_filenames{2}, sn);
        if size(sd,1)>1
            for rr=2:size(sd,1), writecell(num2cell(sd(rr,:))', combined_xlsx, 'Sheet','r_only','WriteMode','append'); end
        end
    catch, fprintf('  [警告] 无法读取 %s sheet %s\n', excel_filenames{2}, sn); end
end
fprintf('  Sheet "r_only" OK\n');

% ---- Sheet 3: i+r_combined ----
writecell(col_hdr, combined_xlsx, 'Sheet', 'i_r_combined', 'WriteMode', 'overwritesheet');
for fi = 1:2
    for i_n = 1:length(nations)-1
        sn = strrep(nations(i_n),' ','_');
        try
            [~,~,sd] = xlsread(excel_filenames{fi}, sn);
            if size(sd,1)>1
                for rr=2:size(sd,1), writecell(num2cell(sd(rr,:))', combined_xlsx, 'Sheet','i_r_combined','WriteMode','append'); end
            end
        catch, continue; end
    end
end
fprintf('  Sheet "i_r_combined" OK\n');

% ---- Sheets 4-6: summary ----
sum_hdr = {'Nation', 'Attribute', 'r_model_y_mean', 'rmse_y_mean', 'N'};
sum_names = {'summary_i', 'summary_r', 'summary_i_r'};

for si = 1:3
    writecell(sum_hdr, combined_xlsx, 'Sheet', sum_names{si}, 'WriteMode', 'overwritesheet');
    for i_n = 1:length(nations)-1
        nation = nations(i_n);
        sn = strrep(nation,' ','_');
        r_vals=[]; rmse_vals=[];
        src_range = [];
        if si==1, src_range = 1;
        elseif si==2, src_range = 2;
        else, src_range = [1 2];
        end
        for ri = src_range
            try
                [~,~,sd] = xlsread(excel_filenames{ri}, sn);
                if size(sd,1)>1
                    for rr=2:size(sd,1)
                        v=sd{rr,4}; if isnumeric(v)&&~isnan(v), r_vals=[r_vals;v]; end
                        v2=sd{rr,5}; if isnumeric(v2)&&~isnan(v2), rmse_vals=[rmse_vals;v2]; end
                    end
                end
            catch, continue; end
        end
        if ~isempty(r_vals)
            writecell({nation,'',nanmean(r_vals),nanmean(rmse_vals),length(r_vals)}, ...
                combined_xlsx, 'Sheet', sum_names{si}, 'WriteMode', 'append');
        end
    end
    fprintf('  Sheet "%s" OK\n', sum_names{si});
end
...
%% ===== 最终汇总打印 =====
fprintf('\n\n========== N-Drop CV 最终汇总 (n_drop=%d, iOr=i+r) ==========\n', n_drop);
for pp = 1:2
    cur_iOr = iOr_list{pp};
    fprintf('\n--- iOr = "%s" ---\n', cur_iOr);
    for i_n = 1:length(nations)
        ...
        sn = strrep(nation,' ','_');
        try
            [~,~,sheet_data] = xlsread(excel_filenames{pp}, sn);
        catch, continue; end
        ...
        for ia = 1:length(attributes)
            an = attribute_names_new(attributes(ia));
            ryl=[]; rmsyl=[]; rcml=[]; rcel=[];
            for rr=2:size(sheet_data,1)
                if ischar(sheet_data{rr,1})&&strcmp(strtrim(sheet_data{rr,1}),an)
                    v=sheet_data{rr,4}; if isnumeric(v)&&~isnan(v), ryl=[ryl;v]; end
                    v2=sheet_data{rr,5}; if isnumeric(v2)&&~isnan(v2), rmsyl=[rmsyl;v2]; end
                    v3=sheet_data{rr,6}; if isnumeric(v3)&&~isnan(v3), rcml=[rcml;v3]; end
                    v4=sheet_data{rr,7}; if isnumeric(v4)&&~isnan(v4), rcel=[rcel;v4]; end
                end
            end
            if ~isempty(ryl)
                fprintf('  %-12s | %10.4f | %8.4f | %8.4f | %8.4f | %d\n', ...
                    an, nanmean(ryl), nanmean(rmsyl), nanmean(rcml), nanmean(rcel), length(ryl));
            end
        end
    end
end
...
fprintf('\n\n所有输出:\n');
for ii=1:length(iOr_list)
    fprintf('  iOr="%s": %s\n', iOr_list{ii}, excel_filenames{ii});
end
fprintf('  combined: %s\n', combined_xlsx);
fprintf('  pics: %s\n', pic_folder);
fprintf('\n========== Done! ==========\n');
