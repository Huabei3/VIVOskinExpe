...
for i_obs = 1:length(obs_types)
    obs_type = obs_types(i_obs);
    r_excel_output_folder = fullfile("AnalyseResults_p",Dtype,scale_type_origin,"model_fullpara",CT_type,"new", iOr, obs_type);
    ...
    for idx_attribute = 1:length(attributes) % 使用新的索引来遍历 attributes 数组
        ...
        % 保存拟合参数
        output_folder_params = fullfile('AnalyseResults_p', Dtype,scale_type_origin,"model_fullpara",CT_type,"new", iOr, obs_type);
