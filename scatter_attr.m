...
for i_obs=1:length(obs_types)
    ...
    for i_nation = 1:length(nations)
        ...
        for i_gender = 1:length(genders)
            ...
            % 添加PMCC点
            lab = lab_fit_reshaped{i_obs,i_nation}(indices_target, :, idx_gender, 1);
            lab = nanmean(lab, 3); % 按受试者维度求平均
            lab = nanmean(lab, 1); % 按光源/环境维度求平均
            ...
            exportgraphics(h1, fullfile(save_folder, ...
                strcat(nation_serial, '_', gender, '_scatter_attr.jpg')), ...
                'Resolution', 300);
            close(h1);
            ...
            xlabel('{\ith}');
            ylabel('{\itC*}');
            title([nation_names(i_nation), ' ', gender_names(i_gender)]);
            ...
            % 保存图片
            save_folder1 = fullfile("ellip_pic", Dtype,"attr", obs_type,iOr,"C_h");
