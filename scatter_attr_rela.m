...
for i_nation = 1:length(nations)
    % if i_nation==4
        ...
        nation_limits(i_nation).lim_min_x = -5;
        nation_limits(i_nation).lim_max_x = 5;
        nation_limits(i_nation).lim_min_y = -5;
        nation_limits(i_nation).lim_max_y = 5;
    % else
    %     nation_limits(i_nation).lim_min_x = -5;
    %     nation_limits(i_nation).lim_max_x = 5;
    %     nation_limits(i_nation).lim_min_y = -5;
...
for i_obs=1:length(obs_types)
    ...
    for i_nation = 1:length(nations)
        ...
        for attribute = attributes
            ...
            lab_ref = lab_fit_reshaped{i_obs,i_nation}(indices_target, :, :, 1);
            lab_ref = nanmean(lab_ref, 3); 
            lab_ref = nanmean(lab_ref, 1); % 按光源/环境维度求平均
            lab = lab_fit_reshaped{i_obs,i_nation}(indices_target, :, :, attribute);
            lab = nanmean(lab, 3); 
            lab = nanmean(lab, 1); % 按光源/环境维度求平均
            lab_rela=lab-lab_ref;
            
            scatter(lab_rela(2), lab_rela(3), 30, 'o','filled', ...
                'MarkerFaceColor', colors(attribute, :));
            attribute_char=char(attribute_serial);
            text(lab_rela(2), lab_rela(3), ...
        ...
        labCh_PMCC_rela(i_nation,1:3)=labCh_PMCC(i_nation,1:3)-lab_ref;
        plot(labCh_PMCC_rela(i_nation, 2), labCh_PMCC_rela(i_nation, 3), 's', 'MarkerSize', 8, ...
        ...
        % 保存图片
        save_folder = fullfile("ellip_pic", Dtype,"attr_rela", obs_type,iOr);
        ...
        % 保存图片
        save_folder1 = fullfile("ellip_pic", Dtype,"attr_rela", obs_type,iOr,"C_h");
