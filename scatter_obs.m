...
%% 保存
output_folder=fullfile("ellip_pic_p", Dtype,lightness_type);
...
%%
output_folder=fullfile("AnalyseResults_p",Dtype,scale_type_origin,"sum_list",lightness_type);
if ~exist(output_folder,"dir")
    mkdir(output_folder);
end
...
"lab_fit_reshaped","file_missing","par_reshaped","average_reshaped");
%%
save_path=fullfile("ellip_pic_p", Dtype, "obs","sum_list",lightness_type, iOr);
% sort_lab_fit_XSLX_obs(lab_fit_reshaped, average_mean, save_path, ...
%     indices_target, lightness_type, iOr);
%% 计算全局坐标轴范围
% 初始化极值变量
lim_min_x = 8;  % a*轴最小边界初始化为正无穷
lim_min_y = 8;  % b*轴最小边界初始化为正无穷
lim_max_x = 22; % a*轴最大边界初始化为负无穷
lim_max_y = 22; % b*轴最大边界初始化为负无穷
...
for i_nation = 1:length(nations)
    ...
    % --- 修复：将 Delta E 存储变量的初始化移到 i_nation 循环内 ---
    deltaE_inter_obs = cell(length(obs_types), 1);
    deltaE_vs_ave = cell(length(obs_types), 1);
    deltaE_average_inter_obs = nan(length(obs_types), 1);

    current_nation_lab_g = cell(length(obs_types), length(attributes));
    current_nation_ave = zeros(length(attributes), 3);
    ...
    for i_attr = 1:length(attributes)
        ...
        for i_obs=1:length(obs_types)
            ...
            % 存储当前的 lab_g 数据
            current_nation_lab_g{i_obs, i_attr} = lab_g;
            ...
            lab_g_total{i_obs,i_attr}=lab_g;
    ...
    for i_obs=1:length(obs_types)
        for i=1:length(attributes)
            for j=1:length(attributes)
                dE_attr_mat(i,j)=deltaE2000(lab_g_total{i_obs,i},lab_g_total{i_obs,j});
            end
            dE_ave_obs(i_obs,i_nation,i)=deltaE2000(lab_g_total{i_obs,i},ave);
            scatter(lab_g_total{i_obs,i}( 2), lab_g_total{i_obs,i}( 3), 20, 'o', 'filled', ...
                'MarkerFaceColor', colors(i_obs,:), 'MarkerEdgeColor', colors(i_obs,:), 'LineWidth', 0.5);
            % 标记数据值
            text(lab_g_total{i_obs,i}(2), lab_g_total{i_obs,i}(3), ...
                 num2str(i), 'FontSize', 6, ...
                'VerticalAlignment', 'top', 'Color', 'k');
        end
        dE_attr_mat_all{i_obs,i_nation}=dE_attr_mat;
        dE_attr_obs_ave(i_obs,i_nation)=nanmean(dE_ave_obs(i_obs,i_nation,:));
        dE_attr_obs_inter(i_obs,i_nation)=nanmean(nanmean(dE_attr_mat));
    end
    close(2)
    ...
    % 保存图片
    save_folder = fullfile("ellip_pic_p", Dtype, "obs",lightness_type, iOr);
    if ~exist(save_folder, "dir")
        mkdir(save_folder, 'recursive');
    end
    exportgraphics(gcf, fullfile(save_folder, strcat(nation_serial, '_L.jpg')), 'Resolution', 1500);
    close(gcf);
    ...
    % --- 修复：将 lab_data 的写入移到 i_nation 循环内 ---
