...
for i_obs=1:length(obs_types)
    ...
    for i_nation = 1:length(nations)
        ...
        for attribute = [1]
            ...
            if ~isempty(lab_valid)
                ...
                for i_point = 1:size(lab_valid, 1)
                    ...
                    %     'MarkerFaceColor', colors(i_point,:), 'MarkerEdgeColor', colors(i_point,:), 'LineWidth', 0.5);
                    % 标记数据值
                    text(lab_valid(i_point, 2), lab_valid(i_point, 3), ...
                         picnames_groups(i_point), 'FontSize', 6, ...
                        'VerticalAlignment', 'top', 'Color', colors(i_point,:));
        ...
        % 保存图片
        save_folder = fullfile("ellip_pic", Dtype, "scene",lightness_type, obs_type, iOr);
