...
for i_obs=1:length(obs_types_plot)
    ...
    for i_nation_idx = nation_loop
        for i_attr_idx = attr_loop
            ...
            % 根据variable_type设置figure和title
            if strcmp(variable_type, "nation")
                figure(i_nation);
                current_title = nation_names(i_nation);
                current_save_name = nation_serial;
            else  % attr模式
                figure(attribute);
                current_title = attribute_names(attribute);
                current_save_name = attribute_serial;
            end
            hold on;
            set(gcf, 'Color', 'white');
            ...
            % 只有在if_draw_in_lab==1时才绘制i condition的点
            if if_draw_in_lab == 1
            ...
            title(current_title, 'FontSize', targetFontSize);
            axis equal;
            
            % 根据人种设置不同的坐标轴范围 (nation模式时)
            if strcmp(variable_type, "nation")
                if i_nation == 4  % AF
                    xlim([0, 20]);
                    ylim([0, 20]);
            ...
            img_name = fullfile(save_folder, strcat(current_save_name, '_L.jpg'));
            savefig(gcf, strrep(img_name, 'jpg', 'fig'));
            exportgraphics(gcf, img_name, 'Resolution', 300);
        end  % end for i_attr_idx
    end  % end for i_nation_idx
        ...
        savefig(gcf, strrep(img_name,'jpg','fig'));
        exportgraphics(gcf, img_name, 'Resolution', 300);
        end % end for i_nation_idx (nation mode)
        ...
        opts.axis_limits=repmat([0,28,0,28],10,1);
        opts.axis_ticks=repmat(5,1,10);
        end
        ...
        if strcmp(color_mode, "scene")
            ...
            % 只有在if_draw_in_lab==1时才绘制colorbar
            if if_draw_in_lab == 1
