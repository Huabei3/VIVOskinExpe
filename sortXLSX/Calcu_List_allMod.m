...
obs_type="all";
% 定义保存的汇总 Excel 文件名
summary_folder=fullfile("AnalyseResults", Dtype,"sum_list",obs_type);
summary_filename = fullfile(summary_folder, ...
    'summary_List.xlsx');
...
for lastPartIdx = 1:length(lastParts)
    ...
    % 定义保存的 Excel 文件名
    output_filename = fullfile(summary_folder, lastPart, 'ellip_list_all.xlsx');
    output_filename_no_theta = fullfile(summary_folder, lastPart, 'ellip_list_all_no_theta.xlsx'); % 不包含 theta 的文件
    if ~exist(fullfile(summary_folder, lastPart),"dir")
        mkdir(fullfile(summary_folder, lastPart));
    end
    ...
    % 遍历所有 attribute
    for attribute = [1, 2, 3, 4, 5, 6, 8, 9, 10]
        ...
        save_folder = fullfile("AnalyseResults", Dtype, lastPart, attribute_serial, "list");
        ...
        lambda00 = par_all(:, 1)./(log(1./par_all(:,6)).^2);
        lambda01 = par_all(:, 3)./(log(1./par_all(:,6)).^2) / 2;
        lambda10 = par_all(:, 3) ./(log(1./par_all(:,6)).^2)/ 2;
        lambda11 = par_all(:, 2)./(log(1./par_all(:,6)).^2);
