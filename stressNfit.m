...
for i_lastPart = 1:length(lastParts)
    ...
    for attribute = attributes
        ...
        save_folder=fullfile('AnalyseResults1',Dtype,scale_type);
        ...
        % 处理数据并保存
        process_data_CAT1(dir_res, output_folder, strrep(lastPart,"add",""),Dtype,scale_type);
        ...
        for i_group = 1:length(picnames_groups)
            ...
            if ~ismember(i_group,groups_again)
                ...
                while true
                    % 拟合椭圆
                    [par_mean, r_mean] = calculate_weighted_or_simple_mean( MSV_group_original, lab_group_original);
