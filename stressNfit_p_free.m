...
% for i_lastPart = 1:length(lastParts)
    ...
    for attribute = attributes
        ...
        clear("del_para");i_del=1;del_para={};
        for i_group = 1:length(picnames_groups)
            ...
            if ~ismember(i_group,groups_again)
                ...
                [par_mean, r_mean] = calculate_weighted_or_simple_mean( ...
                    p_group_original, lab_group_original);
                mean_cen(1)=mean(lab_group(:,1));
                    mean_cen(2:3)=par_mean(1,4:5);
                fit_cen(1)=mean(lab_group(:,1));
                [par, r, y] = my_ellipsoidfit3_dy1(lab_group, p_group,mean_cen);                
                fit_cen(2:3)=par(1,4:5);
                p_ratio=sum(p_group>0.5)./length(p_group);
                if deltaE2000(mean_cen,fit_cen)>5||(p_ratio<0.1||p_ratio>0.9)
                    figure(1)
                    plot_contour_with_scatter(par, lab_group, p_group);
                    par=nan(size(par));
                    del_para{i_del,1}=i_group;
                    del_para{i_del,2}=picnames_groups(i_group);
                    i_del=i_del+1;
                ...
                while true
                    ...
                    fit_cen(1)=mean(lab_group(:,1));
                    fit_cen(2:3)=par(1,4:5);
                    deltaE2000(mean_cen,fit_cen),lastPart,picnames_groups(i_group),attribute
                    plot_contour_with_scatter(par, lab_group, p_group);
                    p_ratio=sum(p_group>0.5)./length(p_group);
                    if deltaE2000(mean_cen,fit_cen)>5 ||(p_ratio<0.1||p_ratio>0.9)
                        disp("d")
        ...
        if exist('row_delete_all','var')
            save(fullfile(delete_log_folder, 'row_delete_data.mat'), 'row_delete_all',"del_para");
        end
