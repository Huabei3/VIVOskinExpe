...
function draw_legend_overlay(mainFig, legendPos, targetFontSize, legend_labels, label_Y)
    ...
    for k = 1:numel(labels_row1)
        ...
        % 分段绘制：italic "L" + normal "^*=XX"
        label_str = labels_row1{k};
        label_str=char(label_str);
        % 检查是否需要row1使用斜体（默认不用，除非专门设置row1_italic）
        if isfield(legend_labels, 'row1_italic') && legend_labels.row1_italic
            row1_angle = 'italic';
        else
            row1_angle = 'normal';
        end
        ...
        if ~isempty(caret_pos)
            ...
            ext = get(h_it, 'Extent');
            text(legAx, tx + iconTextGap + ext(3), ty, normal_part, ...
                'FontSize', legendFontSize, 'VerticalAlignment', 'middle', ...
                'Interpreter', legend_interpreter, 'FontAngle', 'normal', ...
                'HorizontalAlignment', 'left');
        else
    ...
    if ~isempty(labels_row2)
        for k = 1:numel(labels_row2)
            ...
            text(legAx, tx + iconTextGap, ty, labels_row2{k}, ...
                'FontSize', legendFontSize, 'VerticalAlignment', 'middle', 'Interpreter', legend_interpreter);
        end
    end
    uistack(legAx, 'top');
end
