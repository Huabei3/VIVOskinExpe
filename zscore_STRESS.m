...
for i_file=1:n_file
    ...
    %zscore标准化
    result(:,2)=(result(:,2)-mean(result(:,2)))./std(result(:,2));
    result(:,2)=(result(:,2)-min(result(:,2)))./(max(result(:,2))-min(result(:,2)));
    repeat_score=[];
    ...
    STRESS_intra(i_file,1)=STRESS(repeat_score(:,2),(repeat_score(:,2)+repeat_score(:,3))./2);
    STRESS_intra1(i_file,1)=STRESS1(repeat_score(:,2),(repeat_score(:,2)+repeat_score(:,3))./2);
    STRESS_intra2(i_file,1)=STRESS2(repeat_score(:,2),(repeat_score(:,2)+repeat_score(:,3))./2);
...
for i_level=10:10:80
    ...
    save(strcat("level_data\labNscore_level",num2str(i_level),".mat"),'lab_level','score_level');
end
%%
%计算STRESS_inter
% score_all_scaled=(score_all-(-3))/6;
score_all_scaled_mean=mean(score_all,2);
for i_file=1:n_file
    STRESS_inter(i_file,1)=STRESS(score_all(:,i_file),score_all_scaled_mean);
    STRESS_inter1(i_file,1)=STRESS1(score_all(:,i_file),score_all_scaled_mean);
end
...
STRESS_inter_mean=mean(STRESS_inter);
STRESS_intra_mean=mean(STRESS_intra);
save("STRESS\STRESS_zscored.mat",'STRESS_inter1','STRESS_inter','STRESS_intra1','STRESS_intra');

%%
%MCDM
for i_level=10:10:80
    load(strcat("level_data\labNscore_level",num2str(i_level),".mat"));
    ...
    load(strcat("level_data\labNscore_level",num2str(i_level),".mat"));
    save(strcat("level_data\labNscore_level_MCDM",num2str(i_level),".mat"), ...
        "preCen_ind","lab_level","score_level","MCDM_inter","preCen");
end

save(strcat("level_data\MCDM_all\labNscore_level_MCDM_all.mat"), ...
...
%     end
% 
%     LG_matrix=ln((cumu_matrix+0.5)./(n_file-cumu_matrix+0.5));
%     z_matrix=0.6422.*LG_matrix+0.0003;
% 
...
% end
% 
% save("fitRes\fitRes_level_z_rec.mat",'par_all','r_all', ...
...
save("fitRes\fitRes_level_z_dire.mat",'par_all','r_all', ...
